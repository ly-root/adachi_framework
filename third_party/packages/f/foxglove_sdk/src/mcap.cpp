...
static size_t customWrite(void *fn, const uint8_t *data, size_t len,
                          int32_t *error) {
  auto *writer = static_cast<CustomWriter *>(fn);
...
foxglove_mcap_options to_c_mcap_options(const McapWriterOptions &options) {
  ...
  c_options.compression =
      static_cast<foxglove_mcap_compression>(options.compression);
  ...
  c_options.calculate_summary_section_crc =
      options.calculate_summary_section_crc;
  ...
  c_options.compression_threads = options.compression_threads.value_or(
      FOXGLOVE_MCAP_COMPRESSION_THREADS_DEFAULT);
  c_options.truncate = options.truncate;
  return c_options;
}
/// @endcond

FoxgloveResult<McapWriter>
McapWriter::create(const McapWriterOptions &options) {
  ...
  if (options.custom_writer.has_value()) {
    custom_writer =
        std::make_unique<CustomWriter>(options.custom_writer.value());
  ...
  if (options.sink_channel_filter) {
    ...
    sink_channel_filter =
        std::make_unique<SinkChannelFilterFn>(options.sink_channel_filter);

    c_options.sink_channel_filter_context = sink_channel_filter.get();
    c_options.sink_channel_filter =
        [](const void *context,
           const struct foxglove_channel_descriptor *channel) -> bool {
      try {
        if (!context) {
          return true;
        }
        const auto *filter_func =
            static_cast<const SinkChannelFilterFn *>(context);
...
FoxgloveError McapWriter::attach(const Attachment &attachment) {
  ...
  c_attachment.media_type = {attachment.media_type.data(),
                             attachment.media_type.length()};
