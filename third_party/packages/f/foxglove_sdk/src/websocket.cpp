...
void forwardOnSubscribe(const void *context, uint64_t channel_id,
                        foxglove_client_metadata c_client_metadata) {
  internal::callbackGuard("onSubscribe", [&] {
    ...
    static_cast<const WebSocketServerCallbacks *>(context)->onSubscribe(
        channel_id, client_metadata);
  });
}

void forwardOnUnsubscribe(const void *context, uint64_t channel_id,
                          foxglove_client_metadata c_client_metadata) {
...
void forwardOnClientAdvertise(const void *context, uint32_t client_id,
                              const foxglove_client_channel *channel) {
  internal::callbackGuard("onClientAdvertise", [&] {
    ClientChannel cpp_channel = {
        ...
        channel->encoding,
        channel->schema_name,
        channel->schema_encoding == nullptr ? std::string_view{}
...
void forwardOnMessageData(const void *context, uint32_t client_id,
                          uint32_t client_channel_id, const uint8_t *payload,
                          size_t payload_len) {
...
void forwardOnClientUnadvertise(uint32_t client_id, uint32_t client_channel_id,
                                const void *context) {
...
void forwardOnPlaybackControlRequest(
  ...
  internal::callbackGuard("onPlaybackControlRequest", [&] {
    ...
    maybe_state =
        ctx->onPlaybackControlRequest(PlaybackControlRequest::from(*c_request));
  });
...
#endif
bool wireServerCallbacks(foxglove_server_callbacks &c,
                         const WebSocketServerCallbacks &cb) {
  bool any = false;
  if (cb.onSubscribe) {
  ...
  if (cb.onGetParameters) {
    c.on_get_parameters =
        &internal::forwardLegacyGetParameters<WebSocketServerCallbacks>;
    any = true;
  }
  if (cb.onSetParameters) {
    c.on_set_parameters =
        &internal::forwardLegacySetParameters<WebSocketServerCallbacks>;
    any = true;
  }
  if (cb.onParametersSubscribe) {
    c.on_parameters_subscribe =
        &internal::forwardParametersSubscribe<WebSocketServerCallbacks>;
    any = true;
  }
  if (cb.onParametersUnsubscribe) {
    c.on_parameters_unsubscribe =
        &internal::forwardParametersUnsubscribe<WebSocketServerCallbacks>;
    any = true;
  }
...
FoxgloveResult<WebSocketServer> WebSocketServer::create(
  ...
  if (wireServerCallbacks(c_callbacks, options.callbacks)) {
    callbacks = std::make_unique<WebSocketServerCallbacks>(
  ...
  if (options.tls_identity) {
    c_options.tls_cert =
        reinterpret_cast<const uint8_t *>(options.tls_identity->cert.data());
    c_options.tls_cert_len = options.tls_identity->cert.size();
    c_options.tls_key =
        reinterpret_cast<const uint8_t *>(options.tls_identity->key.data());
  ...
  if (options.session_id) {
    session_id = foxglove_string{options.session_id->data(),
                                 options.session_id->length()};
...
WebSocketServer::WebSocketServer(
    foxglove_websocket_server *server,
    std::unique_ptr<WebSocketServerCallbacks> callbacks,
    std::unique_ptr<FetchAssetHandler> fetch_asset,
    std::unique_ptr<SinkChannelFilterFn> sink_channel_filter,
    std::unique_ptr<ParameterHandler> parameter_handler)
...
void WebSocketServer::broadcastPlaybackState(
    const PlaybackState &playback_state) const noexcept {
  ...
  if (!playback_state.request_id.has_value() ||
      playback_state.request_id->empty()) {
    c_playback_state_ptr.request_id = {nullptr, 0};
  } else {
    c_playback_state_ptr.request_id = {playback_state.request_id->data(),
                                       playback_state.request_id->length()};
  }
...
FoxgloveError WebSocketServer::clearSession(
    std::optional<std::string_view> session_id) const noexcept {
  auto c_session_id = session_id
                          ? std::optional<foxglove_string>{{session_id->data(),
                                                            session_id->size()}}
                          : std::nullopt;
  auto error = foxglove_server_clear_session(
      impl_.get(), c_session_id ? &*c_session_id : nullptr);
  return FoxgloveError(error);
}
...
FoxgloveError
WebSocketServer::removeService(std::string_view name) const noexcept {
  auto error =
      foxglove_server_remove_service(impl_.get(), {name.data(), name.length()});
  return FoxgloveError(error);
}
...
FoxgloveError
WebSocketServer::removeStatus(const std::vector<std::string_view> &ids) const {
  ...
  auto error =
      foxglove_server_remove_status(impl_.get(), c_ids.data(), c_ids.size());
  return FoxgloveError(error);
}
