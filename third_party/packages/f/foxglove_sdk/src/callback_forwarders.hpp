...
struct ForwarderAccess {
  static FetchAssetResponder
  makeFetchAssetResponder(foxglove_fetch_asset_responder *p) {
    return FetchAssetResponder(p);
  }
  static foxglove_fetch_asset_responder *
  releaseFetchAsset(FetchAssetResponder &r) {
    return r.impl_.release();
  }
  static GetParametersResponder
  makeGetResponder(foxglove_get_parameters_responder *p) {
    return GetParametersResponder(p);
  }
  static SetParametersResponder
  makeSetResponder(foxglove_set_parameters_responder *p) {
...
template <class F> inline void callbackGuard(const char *name, F &&f) noexcept {
  try {
    std::forward<F>(f)();
...
void forwardFetchAsset(const void *context, const foxglove_string *c_uri,
                       foxglove_fetch_asset_responder *c_responder);

bool forwardSinkChannelFilter(const void *context,
                              const foxglove_channel_descriptor *channel);

void forwardParameterHandlerGet(const void *context, uint32_t client_id,
...
void forwardParameterHandlerSet(const void *context, uint32_t client_id,
                                const foxglove_string *c_request_id,
...
void forwardParametersSubscribe(const void *context,
...
template <class Callbacks>
void forwardParametersUnsubscribe(const void *context,
                                  const foxglove_string *c_names,
                                  size_t names_len) {
...
forwardLegacyGetParameters(const void *context, uint32_t client_id,
                           const foxglove_string *c_request_id,
                           const foxglove_string *c_param_names,
                           size_t param_names_len) {
  ...
  callbackGuard("onGetParameters", [&] {
    ...
    auto params = (static_cast<const Callbacks *>(context))
...
template <class Callbacks>
foxglove_parameter_array *
forwardLegacySetParameters(const void *context, uint32_t client_id,
                           const foxglove_string *c_request_id,
                           const foxglove_parameter_array *c_params) {
  if (c_params == nullptr) {
    return nullptr;
  ...
  callbackGuard("onSetParameters", [&] {
    ...
    auto params =
        (static_cast<const Callbacks *>(context))
            ->onSetParameters(client_id, request_id,
                              ParameterArrayView(c_params).parameters());
    auto array = ParameterArray(std::move(params));
    result = ForwarderAccess::releaseParameterArray(array);
  });
  return result;
...
// Wiring helpers. Templated on the C options struct because
// foxglove_server_options and foxglove_gateway_options share field names but
// are distinct types.

template <class COptions>
void wireFetchAsset(COptions &c_options, FetchAssetHandler &&cpp_handler,
                    std::unique_ptr<FetchAssetHandler> &out) {
  if (!cpp_handler) {
    return;
  }
...
template <class COptions>
void wireSinkChannelFilter(COptions &c_options,
                           SinkChannelFilterFn &&cpp_handler,
                           std::unique_ptr<SinkChannelFilterFn> &out) {
  if (!cpp_handler) {
    return;
  }
...
template <class COptions>
FoxgloveError wireParameterHandler(COptions &c_options,
                                   foxglove_parameter_handler &c_handler,
                                   ParameterHandler &&cpp_handler,
