 ...
 */
void ServiceMessageSchema::writeTo(
    foxglove_service_message_schema *c) const noexcept {
  c->encoding = {this->encoding.data(), this->encoding.length()};
  c->schema.name = {this->schema.name.data(), this->schema.name.length()};
  c->schema.encoding = {this->schema.encoding.data(),
                        this->schema.encoding.length()};
 ...
 */
void ServiceSchema::writeTo(foxglove_service_schema *c,
                            std::array<foxglove_service_message_schema, 2>
                                &msg_schemas) const noexcept {
...
ServiceRequest::ServiceRequest(const foxglove_service_request *r) noexcept
    : service_name(r->service_name.data, r->service_name.len),
      client_id(r->client_id), call_id(r->call_id),
      encoding(r->encoding.data, r->encoding.len),
      payload(reinterpret_cast<const std::byte *>(r->payload.data),
 ...
 */
void ServiceResponder::Deleter::operator()(
    foxglove_service_responder *ptr) const noexcept {
...
void ServiceResponder::respondOk(const std::byte *data,
                                 size_t size) && noexcept {
  auto *ptr = impl_.release();
  foxglove_service_respond_ok(ptr,
 ...
 */
FoxgloveResult<Service> Service::create(std::string_view name,
  ...
  auto error = foxglove_service_create(
      &ptr, {name.data(), name.length()}, &c_schema, &handler,
      [](const void *context, const foxglove_service_request *c_request,
...
foxglove_service *Service::release() noexcept { return impl_.release(); }
