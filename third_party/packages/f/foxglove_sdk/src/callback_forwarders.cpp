...
void forwardFetchAsset(const void *context, const foxglove_string *c_uri,
  ...
  try {
    ...
    if (auto *ptr = ForwarderAccess::releaseFetchAsset(responder)) {
      foxglove_fetch_asset_respond_error(
          ptr, {kErrMessage.data(), kErrMessage.size()});
    }
  }
}
...
void forwardParameterHandlerGet(
    ...
    const foxglove_string *c_request_id, const foxglove_string *c_param_names,
    size_t param_names_len, foxglove_get_parameters_responder *c_responder) {
  // Take ownership of the responder before any throwable work, so its
  // destructor sends the generic error to the client if anything below throws.
