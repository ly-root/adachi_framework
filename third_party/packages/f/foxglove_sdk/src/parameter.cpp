...
ParameterValueView::Value ParameterValueView::value() const {
  ...
  case FOXGLOVE_PARAMETER_VALUE_TAG_FLOAT64:
    return impl_->data
        .float64; // NOLINT(cppcoreguidelines-pro-type-union-access)
  case FOXGLOVE_PARAMETER_VALUE_TAG_INTEGER:
    return impl_->data
        .integer; // NOLINT(cppcoreguidelines-pro-type-union-access)
  case FOXGLOVE_PARAMETER_VALUE_TAG_BOOLEAN:
    return impl_->data
        .boolean; // NOLINT(cppcoreguidelines-pro-type-union-access)
 ...
 */
void ParameterValue::Deleter::operator()(
    foxglove_parameter_value *ptr) const noexcept {
  foxglove_parameter_value_free(ptr);
}

ParameterValue::ParameterValue(foxglove_parameter_value *ptr) : impl_(ptr) {}

ParameterValue::ParameterValue(double value) : impl_(nullptr) {
  foxglove_parameter_value *ptr =
      foxglove_parameter_value_create_float64(value);
  if (ptr == nullptr) {
    throw std::runtime_error("allocation failed");
  }
  impl_.reset(ptr);
...
ParameterValue::ParameterValue(int64_t value) : impl_(nullptr) {
  foxglove_parameter_value *ptr =
      foxglove_parameter_value_create_integer(value);
  if (ptr == nullptr) {
    throw std::runtime_error("allocation failed");
  }
  impl_.reset(ptr);
...
ParameterValue::ParameterValue(bool value) : impl_(nullptr) {
  foxglove_parameter_value *ptr =
      foxglove_parameter_value_create_boolean(value);
  if (ptr == nullptr) {
    throw std::runtime_error("allocation failed");
  }
  impl_.reset(ptr);
...
ParameterValue::ParameterValue(std::string_view value) : impl_(nullptr) {
  foxglove_parameter_value *ptr = nullptr;
  auto error = foxglove_parameter_value_create_string(
      &ptr, {value.data(), value.length()});
...
ParameterValue::ParameterValue(std::vector<ParameterValue> values)
    : impl_(nullptr) {
  foxglove_parameter_value_array *array_ptr =
      foxglove_parameter_value_array_create(values.size());
...
ParameterValue::ParameterValue(std::map<std::string, ParameterValue> value)
    : impl_(nullptr) {
  foxglove_parameter_value_dict *dict_ptr =
      foxglove_parameter_value_dict_create(value.size());
  if (dict_ptr == nullptr) {
    throw std::runtime_error("allocation failed");
  }
  ...
  for (auto &pair : value) {
    ...
    auto error = foxglove_parameter_value_dict_insert(
        dict_ptr, {key.data(), key.length()}, value_ptr);
    if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
  ...
  foxglove_parameter_value *ptr =
      foxglove_parameter_value_create_dict(dict_ptr);
  if (ptr == nullptr) {
    throw std::runtime_error("allocation failed");
  }
...
FoxgloveResult<std::vector<std::byte>> ParameterView::getByteArray() const {
  ...
  bytes.resize(len);
  error = foxglove_parameter_decode_byte_array(
      impl_, reinterpret_cast<uint8_t *>(bytes.data()), &len);
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    return tl::unexpected(FoxgloveError(error));
  }
  bytes.resize(len);
...
Parameter::Parameter(foxglove_parameter *param) : impl_(param) {}

Parameter::Parameter(std::string_view name) : impl_(nullptr) {
  foxglove_parameter *ptr = nullptr;
  auto error =
      foxglove_parameter_create_empty(&ptr, {name.data(), name.length()});
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    throw std::runtime_error(foxglove_error_to_cstr(error));
  }
  impl_.reset(ptr);
...
Parameter::Parameter(std::string_view name, bool value) : impl_(nullptr) {
  foxglove_parameter *ptr = nullptr;
  auto error = foxglove_parameter_create_boolean(
      &ptr, {name.data(), name.length()}, value);
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    throw std::runtime_error(foxglove_error_to_cstr(error));
  }
  impl_.reset(ptr);
...
Parameter::Parameter(std::string_view name, double value) : impl_(nullptr) {
  foxglove_parameter *ptr = nullptr;
  auto error = foxglove_parameter_create_float64(
      &ptr, {name.data(), name.length()}, value);
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    throw std::runtime_error(foxglove_error_to_cstr(error));
  }
...
Parameter::Parameter(std::string_view name, int64_t value) : impl_(nullptr) {
  foxglove_parameter *ptr = nullptr;
  auto error = foxglove_parameter_create_integer(
      &ptr, {name.data(), name.length()}, value);
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    throw std::runtime_error(foxglove_error_to_cstr(error));
  }
...
Parameter::Parameter(std::string_view name, const uint8_t *data,
                     size_t data_length)
    : impl_(nullptr) {
  foxglove_parameter *ptr = nullptr;
  auto error = foxglove_parameter_create_byte_array(
      &ptr, {name.data(), name.length()}, {data, data_length});
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    throw std::runtime_error(foxglove_error_to_cstr(error));
  }
  impl_.reset(ptr);
...
Parameter::Parameter(std::string_view name,
    ...
    : Parameter::Parameter(name, ParameterType::None,
                           ParameterValue(std::move(values))) {}

Parameter::Parameter(std::string_view name, ParameterType type,
  ...
  auto error = foxglove_parameter_create(
      &ptr, {name.data(), name.length()},
      static_cast<foxglove_parameter_type>(type), value_ptr);
  if (error != foxglove_error::FOXGLOVE_ERROR_OK) {
    throw std::runtime_error(foxglove_error_to_cstr(error));
  }
  impl_.reset(ptr);
...
foxglove_parameter *Parameter::release() noexcept { return impl_.release(); }
 ...
 */
void ParameterArray::Deleter::operator()(
    foxglove_parameter_array *ptr) const noexcept {
...
ParameterArray::ParameterArray(std::vector<Parameter> &&params)
    : impl_(nullptr) {
  foxglove_parameter_array *array_ptr =
      foxglove_parameter_array_create(params.size());
  if (array_ptr == nullptr) {
    throw std::runtime_error("allocation failed");
  }
