...
// foxglove_cpp_shared target. The guard lets this file still compile cleanly if
// it's invoked outside cmake (or by a consumer that hand-rolls the wrapper
// build), where foxglove-c/foxglove-c.h's RA-guarded declarations would
...
void forwardOnConnectionStatusChanged(const void *context,
                                      foxglove_connection_status status) {
...
void forwardOnSubscribe(const void *context, uint32_t client_id,
                        const foxglove_channel_descriptor *channel) {
  internal::callbackGuard("onSubscribe", [&] {
    auto cpp_channel = ChannelDescriptor(channel);
    static_cast<const RemoteAccessGatewayCallbacks *>(context)->onSubscribe(
        client_id, cpp_channel);
  });
}

void forwardOnUnsubscribe(const void *context, uint32_t client_id,
                          const foxglove_channel_descriptor *channel) {
...
void forwardOnMessageData(const void *context, uint32_t client_id,
                          const foxglove_channel_descriptor *channel,
                          const uint8_t *payload, size_t payload_len) {
  internal::callbackGuard("onMessageData", [&] {
    auto cpp_channel = ChannelDescriptor(channel);
    static_cast<const RemoteAccessGatewayCallbacks *>(context)->onMessageData(
        client_id, cpp_channel, reinterpret_cast<const std::byte *>(payload),
        payload_len);
  });
}

void forwardOnClientAdvertise(const void *context, uint32_t client_id,
                              const foxglove_channel_descriptor *channel) {
...
void forwardOnClientUnadvertise(const void *context, uint32_t client_id,
                                const foxglove_channel_descriptor *channel) {
...
forwardQosClassifier(const void *context,
                     const foxglove_channel_descriptor *channel) {
  if (context == nullptr) {
...
#endif
bool wireGatewayCallbacks(foxglove_gateway_callbacks &c,
                          const RemoteAccessGatewayCallbacks &cb) {
  bool any = false;
  ...
  if (cb.onGetParameters) {
    c.on_get_parameters =
        &internal::forwardLegacyGetParameters<RemoteAccessGatewayCallbacks>;
    any = true;
  }
  if (cb.onSetParameters) {
    c.on_set_parameters =
        &internal::forwardLegacySetParameters<RemoteAccessGatewayCallbacks>;
    any = true;
  }
  if (cb.onParametersSubscribe) {
    c.on_parameters_subscribe =
        &internal::forwardParametersSubscribe<RemoteAccessGatewayCallbacks>;
    any = true;
  }
  ...
  if (cb.onConnectionGraphSubscribe) {
    c.on_connection_graph_subscribe =
        &internal::forwardConnectionGraphSubscribe<
            RemoteAccessGatewayCallbacks>;
    any = true;
  }
  if (cb.onConnectionGraphUnsubscribe) {
    c.on_connection_graph_unsubscribe =
        &internal::forwardConnectionGraphUnsubscribe<
            RemoteAccessGatewayCallbacks>;
    any = true;
  }
  return any;
...
FoxgloveResult<RemoteAccessGateway> RemoteAccessGateway::create(
  ...
  if (wireGatewayCallbacks(c_callbacks, options.callbacks)) {
    callbacks = std::make_unique<RemoteAccessGatewayCallbacks>(
        std::move(options.callbacks));
  ...
  c_options.device_token = {options.device_token.c_str(),
                            options.device_token.length()};
  c_options.callbacks = callbacks ? &c_callbacks : nullptr;
  c_options.capabilities = {
      static_cast<std::underlying_type_t<decltype(options.capabilities)>>(
  ...
  if (options.qos_classifier) {
    qos_classifier =
        std::make_unique<QosClassifierFn>(std::move(options.qos_classifier));
    c_options.qos_classifier_context = qos_classifier.get();
    c_options.qos_classifier = &forwardQosClassifier;
  }

  // Fetch asset handler
  internal::wireFetchAsset(c_options, std::move(options.fetch_asset),
                           fetch_asset);

  foxglove_parameter_handler c_parameter_handler = {};
  if (auto err = internal::wireParameterHandler(
          c_options, c_parameter_handler, std::move(options.parameter_handler),
          parameter_handler);
  ...
  if (options.foxglove_api_url) {
    api_url = {options.foxglove_api_url->c_str(),
               options.foxglove_api_url->length()};
  ...
  if (options.server_info) {
    ...
    for (auto const &[key, value] : *options.server_info) {
      server_info.push_back(
          {{key.data(), key.length()}, {value.data(), value.length()}});
    }
  ...
  return RemoteAccessGateway(
      gateway, std::move(callbacks), std::move(fetch_asset),
      std::move(sink_channel_filter), std::move(qos_classifier),
      std::move(parameter_handler));
}

RemoteAccessGateway::RemoteAccessGateway(
    foxglove_gateway *gateway,
    std::unique_ptr<RemoteAccessGatewayCallbacks> callbacks,
    std::unique_ptr<FetchAssetHandler> fetch_asset,
    std::unique_ptr<SinkChannelFilterFn> sink_channel_filter,
    std::unique_ptr<QosClassifierFn> qos_classifier,
    std::unique_ptr<ParameterHandler> parameter_handler)
    : callbacks_(std::move(callbacks)), fetch_asset_(std::move(fetch_asset)),
...
RemoteAccessConnectionStatus RemoteAccessGateway::connectionStatus() const {
  return static_cast<RemoteAccessConnectionStatus>(
      foxglove_gateway_connection_status(impl_.get()));
}

// NOLINTNEXTLINE(cppcoreguidelines-rvalue-reference-param-not-moved)
FoxgloveError
RemoteAccessGateway::addService(Service &&service) const noexcept {
  auto error = foxglove_gateway_add_service(impl_.get(), service.release());
  return FoxgloveError(error);
}

FoxgloveError
RemoteAccessGateway::removeService(std::string_view name) const noexcept {
  foxglove_string c_name = {name.data(), name.length()};
  auto error = foxglove_gateway_remove_service(impl_.get(), c_name);
  return FoxgloveError(error);
}

void RemoteAccessGateway::publishParameterValues(
...
FoxgloveError RemoteAccessGateway::publishStatus(
    ...
    std::optional<std::string_view> id) const noexcept {
  auto c_id = id ? std::optional<foxglove_string>{{id->data(), id->size()}}
                 : std::nullopt;
  auto error = foxglove_gateway_publish_status(
      impl_.get(), static_cast<foxglove_server_status_level>(level),
      {message.data(), message.size()}, c_id ? &*c_id : nullptr);
  return FoxgloveError(error);
}

FoxgloveError RemoteAccessGateway::removeStatus(
    const std::vector<std::string_view> &ids) const {
  ...
  auto error =
      foxglove_gateway_remove_status(impl_.get(), c_ids.data(), c_ids.size());
  return FoxgloveError(error);
}

FoxgloveError RemoteAccessGateway::publishConnectionGraph(
    const ConnectionGraph &graph) const {
  return FoxgloveError(foxglove_gateway_publish_connection_graph(
      impl_.get(), graph.impl_.get()));
}
