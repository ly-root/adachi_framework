...
FoxgloveResult<SystemInfoPublisher> SystemInfoPublisher::create(
  ...
  if (options.topic) {
    c_options.topic =
        foxglove_string{options.topic->data(), options.topic->length()};
  }

  std::optional<uint64_t> refresh_interval_ms;
  if (options.refresh_interval) {
    refresh_interval_ms = static_cast<uint64_t>(
        std::max<int64_t>(options.refresh_interval->count(), 0));
    c_options.refresh_interval_ms = &*refresh_interval_ms;
  }

  foxglove_system_info_publisher *publisher = nullptr;
  foxglove_error error =
      foxglove_system_info_publisher_start(&c_options, &publisher);
...
FoxgloveError SystemInfoPublisher::stop() noexcept {
  if (auto *impl = impl_.release()) {
    return static_cast<FoxgloveError>(
        foxglove_system_info_publisher_stop(impl));
  }
