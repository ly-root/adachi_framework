...
std::string_view ChannelDescriptor::topic() const noexcept {
  foxglove_string topic =
      foxglove_channel_descriptor_get_topic(channel_descriptor_);
...
ChannelDescriptor::metadata() const noexcept {
  ...
  auto *iter =
      foxglove_channel_descriptor_metadata_iter_create(channel_descriptor_);
  if (iter == nullptr) {
    return std::nullopt;
  }
...
std::optional<Schema> ChannelDescriptor::schema() const noexcept {
  ...
  foxglove_error error =
      foxglove_channel_descriptor_get_schema(channel_descriptor_, &c_schema);
...
FoxgloveResult<RawChannel>
RawChannel::create(const std::string_view &topic,
  ...
  std::vector<foxglove_key_value>
      metadata_items; // Keep this alive for the duration of the call
  if (metadata) {
  ...
  foxglove_error error = foxglove_raw_channel_create(
      ...
      schema ? &c_schema : nullptr, context.getInner(),
      metadata ? &c_metadata : nullptr, &channel);
...
RawChannel::RawChannel(const foxglove_channel *channel) : impl_(channel) {}

void RawChannel::close() noexcept { foxglove_channel_close(impl_.get()); }
...
std::optional<std::map<std::string, std::string>>
RawChannel::metadata() const noexcept {
  std::map<std::string, std::string> result;

  foxglove_channel_metadata_iterator *iter =
      foxglove_channel_metadata_iter_create(impl_.get());
  if (iter == nullptr) {
    return std::nullopt;
  }
  ...
  while (foxglove_channel_metadata_iter_next(iter, &item)) {
    result[std::string(item.key.data, item.key.len)] =
        std::string(item.value.data, item.value.len);
  }
...
FoxgloveError RawChannel::log(const std::byte *data, size_t data_len,
                              std::optional<uint64_t> log_time,
                              std::optional<uint64_t> sink_id) noexcept {
  foxglove_error error = foxglove_channel_log(
      impl_.get(), reinterpret_cast<const uint8_t *>(data), data_len,
      log_time ? &*log_time : nullptr, sink_id ? *sink_id : 0);
