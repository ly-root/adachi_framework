 ...
 * time. If the server publishes time data, then timestamps of published
 ...
 * requests from controls in the Foxglove app. This requires the server to
 * specify the `data_start_time` and `data_end_time` fields in
 ...
 * A parameter type.
 *
 * This enum is used to disambiguate `foxglove_parameter` values, in situations
 * where the wire representation is ambiguous.
 */
...
#if !defined(__wasm__)
  /**
   * An array of decimal or integer values that can be represented as
   * `float64`s.
   */
 ...
 *
 * Obtained via the `get` callback of `foxglove_parameter_handler`. The
 * implementation **must** complete the request by calling
 * `foxglove_get_parameters_responder_respond` exactly once (pass an empty array
 * if no values are available). It is safe to invoke that function synchronously
 * from the context of the callback. See
 * `foxglove_get_parameters_responder_drop` for the drop-without-responding
 * contract.
 */
typedef struct foxglove_get_parameters_responder
    foxglove_get_parameters_responder;
#endif
 ...
 *
 * Obtained via the `set` callback of `foxglove_parameter_handler`. The
 * implementation **must** complete the request by calling
 * `foxglove_set_parameters_responder_respond` exactly once with the values that
 * were actually applied (pass an empty array if the request could not be
 * handled). It is safe to invoke that function synchronously from the context
 * of the callback. See `foxglove_set_parameters_responder_drop` for the
 * drop-without-responding contract.
 *
 * When the request carried a `request_id`, the values passed to `respond` are
 * echoed back to the requesting client; otherwise the responder does nothing on
 * the wire. The responder does not notify other parameter subscribers, so the
 * implementer is responsible for broadcasting applied updates to subscribers on
 * each sink (for example, via `foxglove_server_publish_parameter_values` and
 * `foxglove_gateway_publish_parameter_values`).
 */
typedef struct foxglove_set_parameters_responder
    foxglove_set_parameters_responder;
#endif
 ...
 *
 * The handle is created by [`foxglove_system_info_publisher_start`]. It is
 * freed by
 * [`foxglove_system_info_publisher_stop`] (which aborts the background task) or
 * by
...
typedef struct foxglove_arrow_primitive {
  /**
   * Position of the arrow's tail and orientation of the arrow. Identity
   * orientation means the arrow points in the +x direction.
   */
 ...
 * See the [MCAP spec](https://mcap.dev/spec#schema-op0x03) for more
 * information.
 */
...
typedef struct foxglove_camera_calibration {
  ...
  /**
   * Frame of reference for the camera. The origin of the frame is the optical
   * center of the camera. +x points to the right in the image, +y points down,
   * and +z points into the plane of the image.
   */
   ...
   *
   * Supported parameters: `plumb_bob` (k1, k2, p1, p2, k3),
   * `rational_polynomial` (k1, k2, p1, p2, k3, k4, k5, k6), and
   * `kannala_brandt` (k1, k2, k3, k4), and `fisheye62` (k0, k1, k2, k3, p0, p1,
   * crit_theta [optional]). `plumb_bob` and `rational_polynomial` models are
   * based on the pinhole model
   * [OpenCV's](https://docs.opencv.org/4.11.0/d9/d0c/group__calib3d.html)
   * [pinhole camera
   * model](https://en.wikipedia.org/wiki/Distortion_%28optics%29#Software_correction).
   * The `kannala_brandt` model matches the [OpenvCV
   * fisheye](https://docs.opencv.org/4.11.0/db/d58/group__calib3d__fisheye.html)
   * model. The `fisheye62` model matches the [Project Aria's Fisheye62
   ...
   * Foxglove also treats K[0] == 0.0 as indicating an uncalibrated camera, and
   ...
   *
   * A rotation matrix aligning the camera coordinate system to the ideal stereo
   * image plane so that epipolar lines in both stereo images are parallel.
   */
  double r[9];
  /**
   ...
   * camera intrinsic matrix for the rectified image.
   *
   * It projects 3D points in the camera coordinate frame to 2D pixel
   * coordinates using the focal lengths (fx', fy') and principal point (cx',
   * cy') - these may differ from the values in K.
   *
   * For monocular cameras, Tx = Ty = 0. Normally, monocular cameras will also
   * have R = the identity and P[1:3,1:3] = K.
   *
   * Foxglove currently does not support displaying stereo images, so Tx and Ty
   * are ignored.
   *
...
typedef struct foxglove_circle_annotation {
   ...
   * The coordinate uses the top-left corner of the top-left pixel of the image
   * as the origin.
   */
  ...
  /**
   * Additional user-provided metadata associated with this annotation. Keys
   * must be unique.
   */
...
typedef struct foxglove_compressed_image {
  ...
  const struct foxglove_timestamp *timestamp;
  /**
   * Frame of reference for the image. The origin of the frame is the optical
   * center of the camera. +x points to the right in the image, +y points down,
   * and +z points into the plane of the image.
   */
  struct foxglove_string frame_id;
...
/**
 * A compressed point cloud. A decoder for `format` must decompress `data`,
 * using metadata stored in the compressed payload to recover point positions
 * and any additional per-point attributes. The decoded point cloud must include
 * at least 2 coordinate fields from `x`, `y`, and `z`; `red`, `green`, `blue`,
 * and `alpha` are optional for customizing each point's color.
 */
typedef struct foxglove_compressed_point_cloud {
  ...
  const struct foxglove_pose *pose;
  /**
   * Compressed point cloud data for exactly one point cloud, including any
   * format-specific metadata needed to describe the decoded point attributes.
   */
  const unsigned char *data;
  size_t data_len;
  /**
...
typedef struct foxglove_compressed_video {
   ...
   *
   * The origin of the frame is the optical center of the camera. +x points to
   * the right in the video, +y points down, and +z points into the plane of the
   * video.
   */
  struct foxglove_string frame_id;
  /**
   ...
   *   - Each CompressedVideo message should contain enough OBUs to decode
   * exactly one video frame
   *   - Each message containing a key frame must also include a Sequence Header
   * OBU
   */
  const unsigned char *data;
  size_t data_len;
   ...
   * See more about [H.265 support](https://caniuse.com/hevc), [VP9
   * support](https://caniuse.com/webm), and [AV1
...
typedef struct foxglove_cylinder_primitive {
  /**
   * Position of the center of the cylinder and orientation of the cylinder. The
   * flat face(s) are perpendicular to the z-axis.
   */
  const struct foxglove_pose *pose;
  /**
  ...
  /**
   * 0-1, ratio of the diameter of the cylinder's bottom face (min z) to the
   * bottom of the bounding box
   */
  double bottom_scale;
  /**
   * 0-1, ratio of the diameter of the cylinder's top face (max z) to the top of
   * the bounding box
   */
  double top_scale;
  /**
...
/**
 * A transform between two reference frames in 3D space. The transform defines
 * the position and orientation of a child frame within a parent frame.
 * Translation moves the origin of the child frame relative to the parent
 * origin. The rotation changes the orientation of the child frame around its
 * origin.
 *
 * Examples:
 *
 * - With translation (x=1, y=0, z=0) and identity rotation (x=0, y=0, z=0,
 * w=1), a point at (x=0, y=0, z=0) in the child frame maps to (x=1, y=0, z=0)
 * in the parent frame.
 *
 * - With translation (x=1, y=2, z=0) and a 90-degree rotation around the z-axis
 * (x=0, y=0, z=0.707, w=0.707), a point at (x=1, y=0, z=0) in the child frame
 * maps to (x=-1, y=3, z=0) in the parent frame.
 */
typedef struct foxglove_frame_transform {
  ...
  /**
   * Translation component of the transform, representing the position of the
   * child frame's origin in the parent frame.
   */
  const struct foxglove_vector3 *translation;
  /**
   * Rotation component of the transform, representing the orientation of the
   * child frame in the parent frame
   */
...
typedef struct foxglove_packed_element_field {
  ...
  uint32_t offset;
  /**
   * Type of data in the field. Integers are stored using little-endian byte
   * order.
   */
...
typedef struct foxglove_grid {
  ...
  struct foxglove_string frame_id;
  /**
   * Origin of grid's corner relative to frame of reference; grid is positioned
   * in the x-y plane relative to this origin
   */
  const struct foxglove_pose *pose;
  /**
  ...
  /**
   * Fields in `data`. `red`, `green`, `blue`, and `alpha` are optional for
   * customizing the grid's color. To enable RGB color visualization in the [3D
   ...
   *
   * **note:** All four fields must be present with these exact names for RGB
   * visualization to work. The order of fields doesn't matter, but the names
   * must match exactly.
   *
   ...
   * ```
   *
   * When these fields are present, the 3D panel will offer additional "Color
   * Mode" options including "RGBA (separate fields)" to visualize the RGB data
   * directly. For elevation visualization, set the "Elevation field" to your
   ...
   * For the data element starting at byte offset i, the coordinates of its
   * corner closest to the origin will be:
   *
...
typedef struct foxglove_voxel_grid {
  ...
  struct foxglove_string frame_id;
  /**
   * Origin of the grid’s lower-front-left corner in the reference frame. The
   * grid’s pose is defined relative to this corner, so an untransformed grid
   * with an identity orientation has this corner at the origin.
   */
  const struct foxglove_pose *pose;
  /**
   * Number of grid rows
  ...
  /**
   * Fields in `data`. `red`, `green`, `blue`, and `alpha` are optional for
   * customizing the grid's color.
   */
  const struct foxglove_packed_element_field *fields;
  size_t fields_count;
  /**
   * Grid cell data, interpreted using `fields`, in depth-major, row-major
   * (Z-Y-X) order. For the data element starting at byte offset i, the
   * coordinates of its corner closest to the origin will be:
   *
...
typedef struct foxglove_points_annotation {
   ...
   * These coordinates use the top-left corner of the top-left pixel of the
   * image as the origin.
   */
  ...
  const struct foxglove_color *outline_color;
  /**
   * Per-point colors, if `type` is `POINTS`, or per-segment stroke colors, if
   * `type` is `LINE_LIST`, `LINE_STRIP` or `LINE_LOOP`.
   */
...
typedef struct foxglove_image_annotations {
  /**
   * Timestamp of the image annotations. When set, individual annotation
  ...
  size_t texts_count;
  /**
   * Additional user-provided metadata associated with the image annotations.
   * Keys must be unique within this object. Per-annotation metadata takes
...
typedef struct foxglove_joint_state {
  ...
  /**
   * Joint effort (force or torque). Nm for revolute joints, N for prismatic
   * joints.
   */
  const double *effort;
...
typedef struct foxglove_laser_scan {
  ...
  struct foxglove_string frame_id;
  /**
   * Origin of scan relative to frame of reference; points are positioned in the
   * x-y plane relative to this origin; angles are interpreted as
...
typedef struct foxglove_line_primitive {
  ...
  double thickness;
  /**
   * Indicates whether `thickness` is a fixed size in screen pixels (true), or
   * specified in world coordinates and scales with distance from the camera
   * (false)
   */
  ...
  size_t colors_count;
  /**
   * Indices into the `points` and `colors` attribute arrays, which can be used
   * to avoid duplicating attribute data.
   *
   * If omitted or empty, indexing will not be used. This default behavior is
   * equivalent to specifying [0, 1, ..., N-1] for the indices (where N is the
...
typedef struct foxglove_location_fix {
  ...
  const struct foxglove_timestamp *timestamp;
  /**
   * Frame for the sensor. Latitude and longitude readings are at the origin of
   * the frame.
   */
  struct foxglove_string frame_id;
  /**
  ...
  /**
   * If `position_covariance` is available, `position_covariance_type` must be
   * set to indicate the type of covariance.
   */
  ...
  const struct foxglove_color *color;
  /**
   * Additional user-provided metadata associated with the location fix. Keys
   * must be unique.
   */
  const struct foxglove_key_value_pair *metadata;
  size_t metadata_count;
...
typedef struct foxglove_scene_entity_deletion {
  /**
   * Timestamp of the deletion. Only matching entities earlier than this
 ...
 *
 * The duration is represented by a count of seconds (which may be negative),
 * and a count of fractional seconds at nanosecond resolution (which are always
 * positive).
 */
...
typedef struct foxglove_triangle_list_primitive {
  ...
  const struct foxglove_pose *pose;
  /**
   * Vertices to use for triangles, interpreted as a list of triples (0-1-2,
   * 3-4-5, ...)
   */
  const struct foxglove_point3 *points;
  size_t points_count;
...
typedef struct foxglove_text_primitive {
  /**
   * Position of the center of the text box and orientation of the text.
   * Identity orientation means the text is oriented in the xy-plane and flows
   * from -x to +x.
   */
  const struct foxglove_pose *pose;
  /**
   * Whether the text should respect `pose.orientation` (false) or always face
   * the camera (true)
   */
  bool billboard;
  /**
  ...
  double font_size;
  /**
   * Indicates whether `font_size` is a fixed size in screen pixels (true), or
   * specified in world coordinates and scales with distance from the camera
   * (false)
   */
  bool scale_invariant;
...
/**
 * A primitive representing a 3D model file loaded from an external URL or
 * embedded data
 */
typedef struct foxglove_model_primitive {
  ...
  const struct foxglove_color *color;
  /**
   * Whether to use the color specified in `color` instead of any materials
   ...
   * of embedded model (e.g. `model/gltf-binary`). Required if `data` is
   * provided instead of `url`. Overrides the inferred media type if `url` is
   * provided.
   */
  struct foxglove_string media_type;
  /**
   * Embedded model. One of `url` or `data` should be non-empty. If `data` is
   * non-empty, `media_type` must be set to indicate the type of the data.
   */
  const unsigned char *data;
  size_t data_len;
...
/**
 * A visual element in a 3D scene. An entity may be composed of multiple
 * primitives which all share the same frame of reference.
 */
typedef struct foxglove_scene_entity {
  ...
  struct foxglove_string frame_id;
  /**
   * Identifier for the entity. A entity will replace any prior entity on the
   * same topic with the same `id`.
   */
  struct foxglove_string id;
  /**
   * Length of time (relative to `timestamp`) after which the entity should be
   * automatically removed. Zero value indicates the entity should remain
   * visible until it is replaced or deleted.
   */
  const struct foxglove_duration *lifetime;
  /**
   * Whether the entity should keep its location in the fixed frame (false) or
   * follow the frame specified in `frame_id` as it moves relative to the fixed
   * frame (true)
   */
  bool frame_locked;
  /**
   * Additional user-provided metadata associated with the entity. Keys must be
   * unique.
   */
  const struct foxglove_key_value_pair *metadata;
  size_t metadata_count;
...
/**
 * An estimate of position, orientation, and velocity for an object or reference
 * frame in 3D space
 */
typedef struct foxglove_odometry {
  ...
  /**
   * Row-major 6x6 covariance matrix (x, y, z, rotation about x, rotation about
   * y, rotation about z). Set to zero if unknown.
   */
  double pose_covariance[36];
  /**
   * Row-major 6x6 covariance matrix (vx, vy, vz, angular rate about x, angular
   * rate about y, angular rate about z). Set to zero if unknown.
   */
  double velocity_covariance[36];
  /**
   * Additional user-provided metadata associated with the odometry message.
   * Keys must be unique.
   */
  const struct foxglove_key_value_pair *metadata;
  size_t metadata_count;
 ...
 * with information like normals, intensity, etc.
 */
typedef struct foxglove_point_cloud {
  ...
  /**
   * Fields in `data`. At least 2 coordinate fields from `x`, `y`, and `z` are
   * required for each point's position; `red`, `green`, `blue`, and `alpha` are
   * optional for customizing each point's color.
   */
  const struct foxglove_packed_element_field *fields;
  size_t fields_count;
  /**
...
typedef struct foxglove_raw_image {
  ...
  uint32_t height;
  /**
   * Encoding of the raw image data. See the `data` field description for
   * supported values.
   */
  struct foxglove_string encoding;
  /**
   * Byte length of a single row. This is usually some multiple of `width`
   ...
   * Raw image data.
   *
   * For each `encoding` value, the `data` field contains image pixel data
   * serialized as follows:
   *
   * - `yuv422` or `uyvy`:
   *   - Pixel colors are decomposed into
   * [Y'UV](https://en.wikipedia.org/wiki/Y%E2%80%B2UV) channels.
   *   - Pixel channel values are represented as unsigned 8-bit integers.
   *   - U and V values are shared between horizontal pairs of pixels. Each pair
   * of output pixels is serialized as [U, Y1, V, Y2].
   ...
   *   - Pixel channel values are represented as unsigned 8-bit integers.
   *   - U and V values are shared between horizontal pairs of pixels. Each pair
   * of output pixels is encoded as [Y1, U, Y2, V].
   *   - `step` must be greater than or equal to `width` * 2.
   * - `nv12`:
   *   - Pixel colors are decomposed into
   * [Y'UV](https://en.wikipedia.org/wiki/Y%E2%80%B2UV) channels using 4:2:0
   * chroma subsampling. The data is stored in
   * [NV12](https://www.kernel.org/doc/html/v4.10/media/uapi/v4l/pixfmt-nv12.html)
   * semi-planar layout with two contiguous planes: a Y (luma) plane followed by
   * an interleaved UV (chroma) plane.
   *   - All channel values are represented as unsigned 8-bit integers.
   *   - Both planes use `step` as their row stride.
   *   - The Y plane contains one luma value per pixel (`step` * `height`
   * bytes).
   *   - The UV plane contains interleaved U, V chroma pairs, subsampled by a
   * factor of 2 in both dimensions (`width`/2 pairs per row, `height`/2 rows,
   * `step` * `height`/2 bytes). Each U, V pair is shared by a 2x2 block of
   * pixels.
   ...
   *   - `step` must be greater than or equal to `width` * 4.
   * - `32FC1`:
   *   - Pixel brightness is represented as a single-channel, 32-bit
   * little-endian IEEE 754 floating-point value, ranging from 0.0 (black)
   * to 1.0 (white).
   *   - `step` must be greater than or equal to `width` * 4.
   * - `bayer_rggb8`, `bayer_bggr8`, `bayer_gbrg8`, or `bayer_grbg8`:
   *   - Pixel colors are decomposed into Red, Blue and Green channels.
   *   - Pixel channel values are represented as unsigned 8-bit integers, and
   * serialized in a 2x2 bayer filter pattern.
   *   - The order of the four letters after `bayer_` determine the layout, so
   * for `bayer_wxyz8` the pattern is:
   *   ```text
   *   w | x
   *   - + -
   *   y | z
   ...
   *   - `step` must be greater than or equal to `width`.
   * - `mono16` or `16UC1`:
   *   - Pixel brightness is represented as 16-bit unsigned little-endian
   * integers. Rendering of these values is controlled in [Image panel color
   * mode
 ...
 * write_fn and flush_fn must be non-null. Seek_fn may be null iff
 * `disable_seeking` is set to true. These function pointers may be called from
 * multiple threads. These functions are called synchronously with respect to
 * each other within the SDK, but these calls are not synchronized with other
 * SDK function calls.
 */
typedef struct foxglove_custom_writer {
   ...
   */
  size_t (*write_fn)(void *context, const uint8_t *data, size_t len,
                     int32_t *error);
  /**
...
typedef struct foxglove_mcap_options {
  /**
   * `context` can be null, or a valid pointer to a context created via
   * `foxglove_context_new`. If it's null, the mcap file will be created with
   * the default context.
   */
  ...
  /**
   * A filter for channels that can be used to subscribe to or unsubscribe from
   * channels.
   *
   * This can be used to omit one or more channels from a sink, but still log
   * all channels to another sink in the same context. Return false to disable
   * logging of this channel.
   *
   * This method is invoked from the client's main poll loop and must not block.
   *
   * # Safety
   * - If provided, the handler callback must be a pointer to the filter
   * callback function, and must remain valid until the MCAP sink is dropped.
   */
  bool (*sink_channel_filter)(
      const void *context, const struct foxglove_channel_descriptor *channel);
...
typedef struct foxglove_gateway_callbacks {
   ...
   */
  void (*on_connection_status_changed)(const void *context,
                                       foxglove_connection_status status);
  /**
   * Callback invoked when a client subscribes to a channel.
   */
  void (*on_subscribe)(const void *context, uint32_t client_id,
                       const struct foxglove_channel_descriptor *channel);
  /**
   ...
   */
  void (*on_unsubscribe)(const void *context, uint32_t client_id,
                         const struct foxglove_channel_descriptor *channel);
  /**
   * Callback invoked when a client message is received.
   */
  void (*on_message_data)(const void *context, uint32_t client_id,
                          const struct foxglove_channel_descriptor *channel,
   ...
   *
   * The `param_names` argument is guaranteed to be non-NULL. These arguments
   * point to buffers that are valid and immutable for the duration of the call.
   * If the callback wishes to store these values, they must be copied out.
   *
   * This function should return the named parameters, or all parameters if
   * `param_names` is empty. The return value must be allocated with
   * `foxglove_parameter_array_create`. Ownership of this value is transferred
   * to the callee. A NULL return value is treated as empty.
   *
   * Deprecated: prefer `foxglove_gateway_options::parameter_handler`. If a
   * `FoxgloveParameterHandler` is registered, it takes precedence and this
   * callback is not invoked.
   */
  struct foxglove_parameter_array *(*on_get_parameters)(
      const void *context, uint32_t client_id,
   ...
   * The `request_id` argument may be NULL.
   *
   * The `params` argument is guaranteed to be non-NULL. These arguments point
   * to buffers that are valid and immutable for the duration of the call. If
   * the callback wishes to store these values, they must be copied out.
   *
   * This function should return the updated parameters. The return value must
   * be allocated with `foxglove_parameter_array_create`. Ownership is
   * transferred to the callee. A NULL return value is treated as empty.
   *
   * Deprecated: prefer `foxglove_gateway_options::parameter_handler`. If a
   * `FoxgloveParameterHandler` is registered, it takes precedence and this
   * callback is not invoked.
   */
  struct foxglove_parameter_array *(*on_set_parameters)(
      const void *context, uint32_t client_id,
      const struct foxglove_string *request_id,
  ...
  void (*on_parameters_unsubscribe)(const void *context,
                                    ...
                                    size_t param_names_len);
  /**
   * Callback invoked when the first client subscribes to connection graph
   * updates.
   *
  ...
  /**
   * Callback invoked when the last client unsubscribes from connection graph
   * updates.
   *
   * Requires `FOXGLOVE_GATEWAY_CAPABILITY_CONNECTION_GRAPH`.
   */
 ...
 *
 * When supplied to `foxglove_server_options` or `foxglove_gateway_options`, the
 * handler takes precedence over the deprecated `on_get_parameters` /
 * `on_set_parameters` callbacks on `foxglove_server_callbacks` /
 * `foxglove_gateway_callbacks`. Registering a handler also automatically
 * advertises the `FOXGLOVE_SERVER_CAPABILITY_PARAMETERS` (or
 * `FOXGLOVE_GATEWAY_CAPABILITY_PARAMETERS`) capability. Subscribe/unsubscribe
 * notifications still go through the `on_parameters_subscribe` /
 * `on_parameters_unsubscribe` callbacks on `foxglove_server_callbacks` /
 * `foxglove_gateway_callbacks`; wire those up separately if you want to be
 * notified.
 *
 * Both `get` and `set` are required: if a handler is supplied with either set
 * to NULL, `foxglove_server_start` / `foxglove_gateway_start` returns
 * `FOXGLOVE_ERROR_VALUE_ERROR`.
 *
 * These methods are invoked from time-sensitive contexts and must not block. If
 * long-running behavior is required, the implementation should hand the
...
typedef struct foxglove_parameter_handler {
   ...
   * The `request_id` argument may be NULL.
   *
   * The `param_names` argument is guaranteed to be non-NULL. The buffer is
   * valid for the duration of this call; if the callback wishes to store these
   * values, it must copy them out.
   *
   * The implementation takes ownership of `responder`; see
   * `FoxgloveGetParametersResponder` for the completion contract.
   */
  void (*get)(const void *context, uint32_t client_id,
              const struct foxglove_string *request_id,
              const struct foxglove_string *param_names, size_t param_names_len,
   ...
   * The `request_id` argument may be NULL.
   *
   * The `params` argument is guaranteed to be non-NULL. The buffer is valid for
   * the duration of this call; if the callback wishes to store these values, it
   * must copy them out.
   *
   * The implementation takes ownership of `responder`; see
   * `FoxgloveSetParametersResponder` for the completion contract, the
   * `request_id` echo behavior, and the implementer's responsibility to
   * broadcast applied updates to other parameter subscribers.
   */
  void (*set)(const void *context, uint32_t client_id,
              const struct foxglove_string *request_id,
              const struct foxglove_parameter_array *params,
 ...
 * - If `supported_encodings` is supplied, all entries must contain valid UTF-8,
 * and `supported_encodings` must have length equal to
 * `supported_encodings_count`.
 * - If `server_info` is supplied, all entries must contain valid UTF-8, and
 * `server_info` must have length equal to `server_info_count`.
 */
typedef struct foxglove_gateway_options {
  /**
   * `context` can be null, or a valid pointer to a context created via
   * `foxglove_context_new`. If it's null, the gateway will be created with the
   * default context.
   */
  const struct foxglove_context *context;
  struct foxglove_string name;
  ...
  /**
   * Optional information about the gateway, which is shared with clients via
   * the ServerInfo message.
   *
   * # Safety
   * - If provided, the `server_info` must be a valid pointer to an array of
   * valid `FoxgloveKeyValue`s with `server_info_count` elements.
   */
   ...
   *
   * Returns a [`FoxgloveQosProfile`] for the given channel, determining how
   * data is delivered. If not set, all channels use the default lossy profile.
   */
  struct foxglove_qos_profile (*qos_classifier)(
      const void *context, const struct foxglove_channel_descriptor *channel);
  /**
   ...
   *
   * This method is invoked from a time-sensitive context and must not block. If
   * blocking or long-running behavior is required, the implementation should
   * return immediately and handle the request asynchronously.
   *
   * The `uri` provided to the callback is only valid for the duration of the
   * callback. If the implementation wishes to retain its data for a longer
   * lifetime, it must copy data out of it.
   *
   * The `responder` provided to the callback represents an unfulfilled
   * response. The implementation must eventually call either
   * `foxglove_fetch_asset_respond_ok` or `foxglove_fetch_asset_respond_error`,
   * exactly once, in order to complete the request. It is safe to invoke these
   * completion functions synchronously from the context of the callback.
   *
   * If provided, the Assets capability will be advertised automatically.
   *
   * # Safety
   * - If provided, the handler callback must be a pointer to the fetch asset
   * callback function, and must remain valid until the gateway is stopped.
   */
  void (*fetch_asset)(const void *context, const struct foxglove_string *uri,
   ...
   * `on_get_parameters` / `on_set_parameters` callbacks on
   * `foxglove_gateway_callbacks`. Registering a handler automatically enables
   * the `FOXGLOVE_GATEWAY_CAPABILITY_PARAMETERS` capability.
   *
...
typedef struct foxglove_playback_control_request {
  ...
  /**
   * Unique string identifier, used to indicate that a PlaybackState is in
   * response to a particular request from the client. Should not be an empty
   * string.
   */
...
typedef struct foxglove_playback_state {
  ...
  float playback_speed;
  /**
   * Whether a seek forward or backward in time triggered this message to be
   * emitted
   */
  bool did_seek;
  /**
   * If this message is being emitted in response to a PlaybackControlRequest
   * message, the request_id from that message. Set this to an empty string if
   * the state of playback has been changed by any other condition.
   */
  struct foxglove_string request_id;
...
typedef struct foxglove_server_callbacks {
  ...
  void (*on_unsubscribe)(const void *context, uint64_t channel_id,
                         struct foxglove_client_metadata client);
  void (*on_client_advertise)(const void *context, uint32_t client_id,
                              const struct foxglove_client_channel *channel);
  void (*on_message_data)(const void *context, uint32_t client_id,
                          uint32_t client_channel_id, const uint8_t *payload,
                          size_t payload_len);
  void (*on_client_unadvertise)(uint32_t client_id, uint32_t client_channel_id,
                                const void *context);
  /**
   * Callback invoked when a client requests parameters.
   *
   ...
   * `param_names` is empty. The return value must be allocated with
   * `foxglove_parameter_array_create`. Ownership of this value is transferred
   * to the callee, who is responsible for freeing it. A NULL return value is
   ...
   *
   * This function should return the updated parameters. The return value must
   * be allocated with `foxglove_parameter_array_create`. Ownership of this
   * value is transferred to the callee, who is responsible for freeing it. A
   * NULL return value is treated as an empty array.
   *
   * All clients subscribed to updates for the returned parameters will be
   * notified. Note that if a returned parameter is unset, it will not be
   ...
   * Requires `FOXGLOVE_CAPABILITY_PARAMETERS`.
   *
   * The `param_names` argument is guaranteed to be non-NULL. This argument
   * points to buffers that are valid and immutable for the duration of the
   * call. If the callback wishes to store these values, they must be copied
   * out.
   */
  void (*on_parameters_subscribe)(const void *context,
                                  const struct foxglove_string *param_names,
   ...
   *
   * `playback_control_request` is an input parameter and guaranteed to be
   * non-NULL. `playback_state` is a non-NULL output pointer to a struct that
   * has already been allocated. The caller should fill its fields with the
   * appropriate state of playback, in response to the input playback control
   * request.
   */
  void (*on_playback_control_request)(
      const void *context,
...
typedef struct foxglove_server_options {
  /**
   * `context` can be null, or a valid pointer to a context created via
   * `foxglove_context_new`. If it's null, the server will be created with the
   * default context.
   */
  const struct foxglove_context *context;
  struct foxglove_string name;
   ...
   * # Safety
   * - If provided, the handler callback must be a pointer to the fetch asset
   * callback function, and must remain valid until the server is stopped.
   */
  void (*fetch_asset)(const void *context, const struct foxglove_string *uri,
                      struct foxglove_fetch_asset_responder *responder);
  /**
  ...
  bool (*sink_channel_filter)(
      const void *context, const struct foxglove_channel_descriptor *channel);
  /**
   * If the server is sending data from a fixed time range, and has the
   * PlaybackControl capability, the start time of the data range.
   */
  const uint64_t *playback_start_time;
  /**
   * If the server is sending data from a fixed time range, and has the
   * PlaybackControl capability, the end time of the data range.
   */
   ...
   *
   * # Safety
   * - If provided, the `session_id` must be a valid pointer to a
   ...
   * When set, this handler takes precedence over the deprecated
   * `on_get_parameters` / `on_set_parameters` callbacks on
   * `foxglove_server_callbacks`. Registering a handler automatically enables
   * the `FOXGLOVE_SERVER_CAPABILITY_PARAMETERS` capability.
   *
   * When provided, both `get` and `set` on the handler are required; otherwise
...
typedef struct foxglove_system_info_publisher_options {
  /**
   * Optional context to publish on. When null, the default global context is
   * used.
   */
  const struct foxglove_context *context;
  /**
   * Optional channel topic name. If `data` is null or `len` is 0, defaults to
   * `/sysinfo`.
   */
 ...
 *
 * # Safety
 * We're trusting the caller that the channel will only be used with this type
 * T.
 */
foxglove_error foxglove_channel_create_arrow_primitive(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 *
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_arrow_primitive.
 */
foxglove_error
foxglove_channel_log_arrow_primitive(const struct foxglove_channel *channel,
 ...
 * If the provided buffer has insufficient capacity, writes the required
 * capacity to *encoded_len and returns FOXGLOVE_ERROR_BUFFER_TOO_SHORT. If the
 * message cannot be encoded, logs the reason to stderr and returns
 * FOXGLOVE_ERROR_ENCODE.
 *
 * # Safety
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error
foxglove_arrow_primitive_encode(const struct foxglove_arrow_primitive *msg,
                                uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_camera_calibration(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_camera_calibration.
 */
foxglove_error foxglove_channel_log_camera_calibration(
    const struct foxglove_channel *channel,
    const struct foxglove_camera_calibration *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_camera_calibration_encode(
    const struct foxglove_camera_calibration *msg, uint8_t *ptr, size_t len,
    size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_circle_annotation(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_circle_annotation.
 */
foxglove_error foxglove_channel_log_circle_annotation(
    const struct foxglove_channel *channel,
    const struct foxglove_circle_annotation *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_circle_annotation_encode(const struct foxglove_circle_annotation *msg,
 ...
 */
foxglove_error
foxglove_channel_create_color(struct foxglove_string topic,
                              const struct foxglove_context *context,
                              const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_color.
 */
foxglove_error
foxglove_channel_log_color(const struct foxglove_channel *channel,
                           const struct foxglove_color *msg,
                           const uint64_t *log_time, FoxgloveSinkId sink_id);
#endif
 ...
 * T.
 */
foxglove_error foxglove_channel_create_compressed_image(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_compressed_image.
 */
foxglove_error foxglove_channel_log_compressed_image(
    const struct foxglove_channel *channel,
    const struct foxglove_compressed_image *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_compressed_image_encode(const struct foxglove_compressed_image *msg,
                                 uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_compressed_point_cloud(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_compressed_point_cloud.
 */
foxglove_error foxglove_channel_log_compressed_point_cloud(
    const struct foxglove_channel *channel,
    const struct foxglove_compressed_point_cloud *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_compressed_point_cloud_encode(
    const struct foxglove_compressed_point_cloud *msg, uint8_t *ptr, size_t len,
    size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_compressed_video(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_compressed_video.
 */
foxglove_error foxglove_channel_log_compressed_video(
    const struct foxglove_channel *channel,
    const struct foxglove_compressed_video *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_compressed_video_encode(const struct foxglove_compressed_video *msg,
                                 uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_cylinder_primitive(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_cylinder_primitive.
 */
foxglove_error foxglove_channel_log_cylinder_primitive(
    const struct foxglove_channel *channel,
    const struct foxglove_cylinder_primitive *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_cylinder_primitive_encode(
    const struct foxglove_cylinder_primitive *msg, uint8_t *ptr, size_t len,
    size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_cube_primitive(struct foxglove_string topic,
                                       const struct foxglove_context *context,
                                       const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_cube_primitive.
 */
foxglove_error
foxglove_channel_log_cube_primitive(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_cube_primitive_encode(const struct foxglove_cube_primitive *msg,
                               uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_frame_transform(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_frame_transform.
 */
foxglove_error
foxglove_channel_log_frame_transform(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_frame_transform_encode(const struct foxglove_frame_transform *msg,
                                uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_frame_transforms(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_frame_transforms.
 */
foxglove_error foxglove_channel_log_frame_transforms(
    const struct foxglove_channel *channel,
    const struct foxglove_frame_transforms *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_frame_transforms_encode(const struct foxglove_frame_transforms *msg,
                                 uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_geo_json(struct foxglove_string topic,
                                 const struct foxglove_context *context,
                                 const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_geo_json.
 */
foxglove_error
foxglove_channel_log_geo_json(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_channel_create_grid(struct foxglove_string topic,
                             const struct foxglove_context *context,
                             const struct foxglove_channel **channel);
 ...
 */
foxglove_error
foxglove_channel_create_voxel_grid(struct foxglove_string topic,
                                   const struct foxglove_context *context,
                                   const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_voxel_grid.
 */
foxglove_error
foxglove_channel_log_voxel_grid(const struct foxglove_channel *channel,
 ...
 * T.
 */
foxglove_error foxglove_channel_create_image_annotations(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_image_annotations.
 */
foxglove_error foxglove_channel_log_image_annotations(
    const struct foxglove_channel *channel,
    const struct foxglove_image_annotations *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_image_annotations_encode(const struct foxglove_image_annotations *msg,
                                  uint8_t *ptr, size_t len,
                                  size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_joint_state(struct foxglove_string topic,
                                    const struct foxglove_context *context,
                                    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_joint_state.
 */
foxglove_error
foxglove_channel_log_joint_state(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_joint_state_encode(const struct foxglove_joint_state *msg,
                            uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_joint_states(struct foxglove_string topic,
                                     const struct foxglove_context *context,
                                     const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_joint_states.
 */
foxglove_error
foxglove_channel_log_joint_states(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_joint_states_encode(const struct foxglove_joint_states *msg,
                             uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_key_value_pair(struct foxglove_string topic,
                                       const struct foxglove_context *context,
                                       const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_key_value_pair.
 */
foxglove_error
foxglove_channel_log_key_value_pair(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_key_value_pair_encode(const struct foxglove_key_value_pair *msg,
                               uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_laser_scan(struct foxglove_string topic,
                                   const struct foxglove_context *context,
                                   const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_laser_scan.
 */
foxglove_error
foxglove_channel_log_laser_scan(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_channel_create_line_primitive(struct foxglove_string topic,
                                       const struct foxglove_context *context,
                                       const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_line_primitive.
 */
foxglove_error
foxglove_channel_log_line_primitive(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_line_primitive_encode(const struct foxglove_line_primitive *msg,
                               uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_location_fix(struct foxglove_string topic,
                                     const struct foxglove_context *context,
                                     const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_location_fix.
 */
foxglove_error
foxglove_channel_log_location_fix(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_location_fix_encode(const struct foxglove_location_fix *msg,
                             uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_location_fixes(struct foxglove_string topic,
                                       const struct foxglove_context *context,
                                       const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_location_fixes.
 */
foxglove_error
foxglove_channel_log_location_fixes(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_location_fixes_encode(const struct foxglove_location_fixes *msg,
                               uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_log(struct foxglove_string topic,
                            const struct foxglove_context *context,
                            const struct foxglove_channel **channel);
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_log_encode(const struct foxglove_log *msg, uint8_t *ptr,
 ...
 * T.
 */
foxglove_error foxglove_channel_create_scene_entity_deletion(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_scene_entity_deletion.
 */
foxglove_error foxglove_channel_log_scene_entity_deletion(
    const struct foxglove_channel *channel,
    const struct foxglove_scene_entity_deletion *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_scene_entity_deletion_encode(
    const struct foxglove_scene_entity_deletion *msg, uint8_t *ptr, size_t len,
    size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_scene_entity(struct foxglove_string topic,
                                     const struct foxglove_context *context,
                                     const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_scene_entity.
 */
foxglove_error
foxglove_channel_log_scene_entity(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_scene_entity_encode(const struct foxglove_scene_entity *msg,
                             uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_scene_update(struct foxglove_string topic,
                                     const struct foxglove_context *context,
                                     const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_scene_update.
 */
foxglove_error
foxglove_channel_log_scene_update(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_scene_update_encode(const struct foxglove_scene_update *msg,
                             uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_model_primitive(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_model_primitive.
 */
foxglove_error
foxglove_channel_log_model_primitive(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_model_primitive_encode(const struct foxglove_model_primitive *msg,
                                uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_odometry(struct foxglove_string topic,
                                 const struct foxglove_context *context,
                                 const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_odometry.
 */
foxglove_error
foxglove_channel_log_odometry(const struct foxglove_channel *channel,
 ...
 * T.
 */
foxglove_error foxglove_channel_create_packed_element_field(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_packed_element_field.
 */
foxglove_error foxglove_channel_log_packed_element_field(
    const struct foxglove_channel *channel,
    const struct foxglove_packed_element_field *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_packed_element_field_encode(
    const struct foxglove_packed_element_field *msg, uint8_t *ptr, size_t len,
    size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_point2(struct foxglove_string topic,
                               const struct foxglove_context *context,
                               const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_point2.
 */
foxglove_error
foxglove_channel_log_point2(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_channel_create_point3(struct foxglove_string topic,
                               const struct foxglove_context *context,
                               const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_point3.
 */
foxglove_error
foxglove_channel_log_point3(const struct foxglove_channel *channel,
 ...
 * T.
 */
foxglove_error foxglove_channel_create_point3_in_frame(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_point3_in_frame.
 */
foxglove_error
foxglove_channel_log_point3_in_frame(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_point3_in_frame_encode(const struct foxglove_point3_in_frame *msg,
                                uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_point_cloud(struct foxglove_string topic,
                                    const struct foxglove_context *context,
                                    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_point_cloud.
 */
foxglove_error
foxglove_channel_log_point_cloud(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_point_cloud_encode(const struct foxglove_point_cloud *msg,
                            uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_points_annotation(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_points_annotation.
 */
foxglove_error foxglove_channel_log_points_annotation(
    const struct foxglove_channel *channel,
    const struct foxglove_points_annotation *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_points_annotation_encode(const struct foxglove_points_annotation *msg,
                                  uint8_t *ptr, size_t len,
                                  size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_pose(struct foxglove_string topic,
                             const struct foxglove_context *context,
                             const struct foxglove_channel **channel);
 ...
 */
foxglove_error
foxglove_channel_create_pose_in_frame(struct foxglove_string topic,
                                      const struct foxglove_context *context,
                                      const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_pose_in_frame.
 */
foxglove_error
foxglove_channel_log_pose_in_frame(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_pose_in_frame_encode(const struct foxglove_pose_in_frame *msg,
                              uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_poses_in_frame(struct foxglove_string topic,
                                       const struct foxglove_context *context,
                                       const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_poses_in_frame.
 */
foxglove_error
foxglove_channel_log_poses_in_frame(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_poses_in_frame_encode(const struct foxglove_poses_in_frame *msg,
                               uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_quaternion(struct foxglove_string topic,
                                   const struct foxglove_context *context,
                                   const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_quaternion.
 */
foxglove_error
foxglove_channel_log_quaternion(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_channel_create_raw_audio(struct foxglove_string topic,
                                  const struct foxglove_context *context,
                                  const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_raw_audio.
 */
foxglove_error
foxglove_channel_log_raw_audio(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_channel_create_raw_image(struct foxglove_string topic,
                                  const struct foxglove_context *context,
                                  const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_raw_image.
 */
foxglove_error
foxglove_channel_log_raw_image(const struct foxglove_channel *channel,
 ...
 * T.
 */
foxglove_error foxglove_channel_create_sphere_primitive(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_sphere_primitive.
 */
foxglove_error foxglove_channel_log_sphere_primitive(
    const struct foxglove_channel *channel,
    const struct foxglove_sphere_primitive *msg, const uint64_t *log_time,
    FoxgloveSinkId sink_id);
#endif
 ...
 */
foxglove_error
foxglove_sphere_primitive_encode(const struct foxglove_sphere_primitive *msg,
                                 uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_text_annotation(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_text_annotation.
 */
foxglove_error
foxglove_channel_log_text_annotation(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_text_annotation_encode(const struct foxglove_text_annotation *msg,
                                uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_text_primitive(struct foxglove_string topic,
                                       const struct foxglove_context *context,
                                       const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_text_primitive.
 */
foxglove_error
foxglove_channel_log_text_primitive(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_text_primitive_encode(const struct foxglove_text_primitive *msg,
                               uint8_t *ptr, size_t len, size_t *encoded_len);
 ...
 * T.
 */
foxglove_error foxglove_channel_create_triangle_list_primitive(
    struct foxglove_string topic, const struct foxglove_context *context,
    const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_triangle_list_primitive.
 */
foxglove_error foxglove_channel_log_triangle_list_primitive(
    const struct foxglove_channel *channel,
 ...
 * ptr must be a valid pointer to a memory region at least len bytes long.
 */
foxglove_error foxglove_triangle_list_primitive_encode(
    const struct foxglove_triangle_list_primitive *msg, uint8_t *ptr,
    size_t len, size_t *encoded_len);
 ...
 */
foxglove_error
foxglove_channel_create_vector2(struct foxglove_string topic,
                                const struct foxglove_context *context,
                                const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_vector2.
 */
foxglove_error
foxglove_channel_log_vector2(const struct foxglove_channel *channel,
 ...
 */
foxglove_error
foxglove_channel_create_vector3(struct foxglove_string topic,
                                const struct foxglove_context *context,
                                const struct foxglove_channel **channel);
 ...
 * # Safety
 * The channel must have been created for this type with
 * foxglove_channel_create_vector3.
 */
foxglove_error
foxglove_channel_log_vector3(const struct foxglove_channel *channel,
...
#if !defined(__wasm__)
/**
 * Returns a `FoxgloveMcapOptions` with defaults matching
 * `mcap::WriteOptions::default()`.
 *
 * The test `test_mcap_options_default_matches_write_options` verifies these
 * stay in sync.
 */
 ...
 *
 * If `custom_writer` is provided, the MCAP data will be written using the
 * provided function pointers instead of to a file.
 *
 ...
 */
foxglove_error
foxglove_mcap_open(const struct foxglove_mcap_options *FOXGLOVE_NONNULL options,
 ...
 *
 * # Safety
 * `writer` must be a valid pointer to a `FoxgloveMcapWriter` created via
 * `foxglove_mcap_open`.
 */
 ...
 * # Safety
 * `writer` must be a valid pointer to a `FoxgloveMcapWriter` created via
 * `foxglove_mcap_open`. `name` must be a valid UTF-8 string. `metadata` must be
 * a valid pointer to an array of `foxglove_key_value` with length
 * `metadata_len`.
 */
 ...
 * # Safety
 * `writer` must be a valid pointer to a `FoxgloveMcapWriter` created via
 * `foxglove_mcap_open`. `attachment` must be a valid pointer to a
 * `FoxgloveMcapAttachment`. The `name` and `media_type` fields of the
 * attachment must be valid UTF-8 strings. The `data` field must be a valid
 * pointer to an array of bytes with length `data_len`, or null if `data_len` is
 * 0.
 */
...
#if !defined(__wasm__)
/**
 * Create a new channel. The channel must later be freed with
 ...
 * `schema` is an optional pointer to a schema. The schema and the data it
 * points to need only remain alive for the duration of this function call (they
 * will be copied). `context` can be null, or a valid pointer to a context
 * created via `foxglove_context_new`. `metadata` can be null, or a valid
 * pointer to a collection of key/value pairs. If keys are duplicated in the
 * collection, the last value for each key will be used. `channel` is an out
 * **FoxgloveChannel pointer, which will be set to the created channel if the
 * function returns success.
 */
foxglove_error
foxglove_raw_channel_create(struct foxglove_string topic,
 ...
 *
 * # Safety
 * `channel` must be a valid pointer to a `foxglove_channel` created via
 * `foxglove_channel_create`. If channel is null, this does nothing.
 */
 ...
 */
struct foxglove_string
foxglove_channel_get_topic(const struct foxglove_channel *channel);
#endif
 ...
 */
struct foxglove_string
foxglove_channel_get_message_encoding(const struct foxglove_channel *channel);
#endif
 ...
 *
 * If the passed channel is null or has no schema, returns
 * `FoxgloveError::ValueError`.
 *
 * # Safety
 * `channel` must be a valid pointer to a `foxglove_channel` created via
 * `foxglove_channel_create`. `schema` must be a valid pointer to a
 * `FoxgloveSchema` struct that will be filled in.
 *
 * The returned value is valid only for the lifetime of the channel.
 */
foxglove_error
foxglove_channel_get_schema(const struct foxglove_channel *channel,
                            struct foxglove_schema *schema);
#endif
 ...
 * # Safety
 * `channel` must be a valid pointer to a `foxglove_channel` created via
 * `foxglove_channel_create`. The channel must remain valid for the lifetime of
 * the iterator.
 */
struct foxglove_channel_metadata_iterator *
foxglove_channel_metadata_iter_create(const struct foxglove_channel *channel);
#endif
 ...
 *
 * Returns true if a pair was found and stored in `key_value`, false if the
 * iterator is exhausted.
 *
 * # Safety
 * `iter` must be a valid pointer to a `FoxgloveChannelMetadataIterator` created
 * via `foxglove_channel_metadata_iter_create`. `key_value` must be a valid
 * pointer to a `FoxgloveKeyValue` that will be filled in. The channel itself
 * must still be valid.
 */
bool foxglove_channel_metadata_iter_next(
    struct foxglove_channel_metadata_iterator *iter,
 ...
 * # Safety
 * `iter` must be a valid pointer to a `FoxgloveChannelMetadataIterator` created
 * via `foxglove_channel_metadata_iter_create`.
 */
void foxglove_channel_metadata_iter_free(
    struct foxglove_channel_metadata_iterator *iter);
#endif
 ...
 *
 * Zero-length messages are supported: pass `data_len == 0` with either a null
 * `data` pointer or any non-null pointer. The contents of `data` are not read
 * when `data_len == 0`.
 *
 * # Safety
 * If `data_len > 0`, `data` must be non-null and the range `[data, data +
 * data_len)` must contain initialized data contained within a single allocated
 * object.
 *
 * `log_time` Some(nanoseconds since epoch timestamp) or None to use the current
 * time.
 */
 ...
 *
 * # Safety
 * `context` must be a valid pointer to a context created via
 * `foxglove_context_new`.
 */
 ...
 */
uint64_t foxglove_channel_descriptor_get_id(
    const struct foxglove_channel_descriptor *channel);
#endif
 ...
 * If the passed channel is null, an empty value is returned.
 *
 * The returned value is valid only for the lifetime of the channel, which is
 * typically the duration of a callback where a descriptor is passed.
 */
struct foxglove_string foxglove_channel_descriptor_get_topic(
    const struct foxglove_channel_descriptor *channel);
#endif
 ...
 * typically the duration of a callback where a descriptor is passed.
 */
struct foxglove_string foxglove_channel_descriptor_get_message_encoding(
    const struct foxglove_channel_descriptor *channel);
#endif
 ...
 * # Safety
 * `channel` must be a valid pointer to a `foxglove_channel_descriptor`.
 * `schema` must be a valid pointer to a `FoxgloveSchema` struct that will be
 * filled in.
 *
 * The returned value is valid only for the lifetime of the channel, which is
 * typically the duration of a callback where a descriptor is passed.
 */
foxglove_error foxglove_channel_descriptor_get_schema(
    const struct foxglove_channel_descriptor *channel,
    struct foxglove_schema *schema);
#endif
 ...
 *
 * You must later free the iterator using
 * foxglove_channel_descriptor_metadata_iter_free.
 *
 ...
 */
struct foxglove_channel_descriptor_metadata_iterator *
 ...
 *
 * # Safety
 * `iter` must be a valid pointer to a
 * `foxglove_channel_descriptor_metadata_iterator` created via
 * `foxglove_channel_descriptor_metadata_iter_create`.
 * `key_value` must be a valid pointer to a `FoxgloveKeyValue` that will be
 * filled in. The channel descriptor itself must still be valid.
 */
bool foxglove_channel_descriptor_metadata_iter_next(
    struct foxglove_channel_descriptor_metadata_iterator *iter,
    struct foxglove_key_value *key_value);
#endif

#if !defined(__wasm__)
/**
 * Free a metadata iterator created via
 * `foxglove_channel_descriptor_metadata_iter_create`.
 *
 * # Safety
 * `iter` must be a valid pointer to a
 ...
 * `foxglove_channel_descriptor_metadata_iter_create`.
 */
void foxglove_channel_descriptor_metadata_iter_free(
    struct foxglove_channel_descriptor_metadata_iterator *iter);
#endif
 ...
 *
 * # Safety
 * `graph` must be a valid pointer to a pointer to a
 * `foxglove_connection_graph`.
 */
foxglove_error
foxglove_connection_graph_create(struct foxglove_connection_graph **graph);
#endif
...
#if !defined(__wasm__)
/**
 * Set a published topic and its associated publisher ids. Overwrites any
 * existing topic with the same name.
 *
 * # Safety
 * `topic`, and each ID in `publisher_ids` must adhere to the safety rules of
 * `foxglove_string`. `publisher_ids_count` must be the number of elements in
 * the `publisher_ids` array.
 *
 * These strings are copied from the pointers, and need only be valid for the
 * duration of this function call.
 */
foxglove_error foxglove_connection_graph_set_published_topic(
    struct foxglove_connection_graph *FOXGLOVE_NONNULL graph,
    struct foxglove_string topic, const struct foxglove_string *publisher_ids,
    size_t publisher_ids_count);
#endif

#if !defined(__wasm__)
/**
 * Set a subscribed topic and its associated subscriber ids. Overwrites any
 * existing topic with the same name.
 *
 * # Safety
 * `topic`, and each ID in `subscriber_ids` must adhere to the safety rules of
 * `foxglove_string`. `subscriber_ids_count` must be the number of elements in
 * the `subscriber_ids` array.
 */
foxglove_error foxglove_connection_graph_set_subscribed_topic(
    struct foxglove_connection_graph *FOXGLOVE_NONNULL graph,
    struct foxglove_string topic, const struct foxglove_string *subscriber_ids,
    size_t subscriber_ids_count);
#endif

#if !defined(__wasm__)
/**
 * Set an advertised service and its associated provider ids. Overwrites any
 * existing service with the same name.
 *
 * # Safety
 * `graph` must be a valid pointer to a `foxglove_connection_graph` created by
 * `foxglove_connection_graph_create`. `service`, and each ID in `provider_ids`
 * must adhere to the safety rules of `FoxgloveString`. `provider_ids_count`
 * must be the number of elements in the `provider_ids` array.
 */
foxglove_error foxglove_connection_graph_set_advertised_service(
    struct foxglove_connection_graph *FOXGLOVE_NONNULL graph,
    struct foxglove_string service, const struct foxglove_string *provider_ids,
 ...
 *
 * # Safety
 * - `responder` must be a pointer to a `foxglove_fetch_asset_responder`
 * obtained via a `fetch_asset` callback. This value is moved into this
 * function, and must not be accessed afterwards.
 * - `data` must be a pointer to the response data. This value is copied by this
 * function.
 */
void foxglove_fetch_asset_respond_ok(
    struct foxglove_fetch_asset_responder *responder,
 ...
 * # Safety
 * - `responder` must be a pointer to a `foxglove_fetch_asset_responder`
 * obtained via the `foxglove_server_options.fetch_asset` callback. This value
 * is moved into this function, and must not accessed afterwards.
 * - `message` must be a valid UTF-8 string. This value is copied by this
 * function.
 */
void foxglove_fetch_asset_respond_error(
    struct foxglove_fetch_asset_responder *responder,
    struct foxglove_string message);
#endif
 ...
 *
 * # Safety
 * - `options` must be a valid pointer to a `FoxgloveGatewayOptions` struct with
 * all fields satisfying the documented safety requirements.
 * - If `server_info` is supplied in options, all `server_info` must contain
 * valid UTF8, and `server_info` must have length equal to `server_info_count`.
 * - `gateway` must be a valid pointer to a `*mut FoxgloveGateway`.
 */
foxglove_error foxglove_gateway_start(
    const struct foxglove_gateway_options *FOXGLOVE_NONNULL options,
 ...
 *
 * Returns `Shutdown` if the gateway pointer is null or the gateway has been
 * stopped.
 */
foxglove_connection_status
foxglove_gateway_connection_status(const struct foxglove_gateway *gateway);
#endif
 ...
 *
 * # Safety
 * - `gateway` must be a valid pointer to a gateway started with
 * `foxglove_gateway_start`.
 * - `service` must be a valid pointer to a service allocated by
 * `foxglove_service_create`. This value is moved into this function, and must
 * not be accessed afterwards.
 */
foxglove_error
foxglove_gateway_add_service(const struct foxglove_gateway *gateway,
 ...
 */
foxglove_error
foxglove_gateway_remove_service(const struct foxglove_gateway *gateway,
                                struct foxglove_string service_name);
#endif
 ...
 * - `gateway` must be a valid pointer to a gateway started with
 * `foxglove_gateway_start`.
 * - `params` must be a valid pointer to a value allocated by
 * `foxglove_parameter_array_create`. This value is moved into this function,
 * and must not be accessed afterwards.
 */
foxglove_error foxglove_gateway_publish_parameter_values(
    const struct foxglove_gateway *gateway,
 ...
 * subsequent call to `foxglove_gateway_remove_status`.
 *
 * # Safety
 * - `gateway` must be a valid pointer to a gateway started with
 * `foxglove_gateway_start`.
 * - `message` must be a valid UTF-8 string, which must remain valid for the
 * duration of this call.
 * - `id` must either be NULL, or a pointer to a valid UTF-8 string, which must
 * remain valid for the duration of this call.
 */
foxglove_error foxglove_gateway_publish_status(
    const struct foxglove_gateway *gateway, foxglove_server_status_level level,
    struct foxglove_string message, const struct foxglove_string *id);
#endif
 ...
 * - `gateway` must be a valid pointer to a gateway started with
 * `foxglove_gateway_start`.
 * - `ids` must be a pointer to an array of valid UTF-8 strings, all of which
 * must remain valid for the duration of this call.
 */
foxglove_error
foxglove_gateway_remove_status(const struct foxglove_gateway *gateway,
 ...
 * Requires `FOXGLOVE_GATEWAY_CAPABILITY_CONNECTION_GRAPH`.
 */
foxglove_error foxglove_gateway_publish_connection_graph(
    const struct foxglove_gateway *gateway,
 ...
 * capture output from all components. Subsequent calls will have no effect.
 *
 * Log level may be overridden with the FOXGLOVE_LOG_LEVEL environment variable:
 * "debug", "info", "warn", "error", or "off". The default level is "info".
 *
 * Log styles (colors) may be configured with the FOXGLOVE_LOG_STYLE environment
 * variable "never", "always", or "auto" (default).
 *
 * This is thread-safe, but only the first call to this function will have an
 * effect.
 */
...
foxglove_parameter_array_create(size_t capacity);
#endif
 ...
 * - `param` must be a valid parameter to a value allocated by
 * `foxglove_parameter_create` or `foxglove_parameter_clone`. This value is
 * moved into this function, and must not be accessed afterwards.
 */
foxglove_error
foxglove_parameter_array_push(struct foxglove_parameter_array *array,
 ...
 *
 * The parameter must be freed with `foxglove_parameter_free`, or by passing it
 * to a consuming function such as `foxglove_parameter_array_push`.
 *
 * # Safety
 * - `param` must be a valid pointer.
 * - `name` must be a valid `foxglove_string`. This value is copied by this
 * function.
 * - `value` must either be a valid pointer to a value allocated by
 *   `foxglove_parameter_value_create`, or NULL. This value is moved into this
 * function, and must not be accessed afterwards.
 */
foxglove_error foxglove_parameter_create(
    struct foxglove_parameter **param, struct foxglove_string name,
    foxglove_parameter_type type, struct foxglove_parameter_value *value);
#endif
 ...
 *
 * The value must be freed with `foxglove_parameter_free`, or by passing it to a
 * consuming function such as `foxglove_parameter_array_push`.
 *
 * # Safety
 * - `param` must be a valid pointer.
 * - `name` must be a valid `foxglove_string`. This value is copied by this
 ...
 */
foxglove_error
foxglove_parameter_create_empty(struct foxglove_parameter **param,
                                struct foxglove_string name);
#endif
 ...
 */
foxglove_error
foxglove_parameter_create_float64(struct foxglove_parameter **param,
                                  struct foxglove_string name, double value);
#endif
 ...
 */
foxglove_error
foxglove_parameter_create_integer(struct foxglove_parameter **param,
                                  struct foxglove_string name, int64_t value);
#endif
 ...
 */
foxglove_error
foxglove_parameter_create_boolean(struct foxglove_parameter **param,
                                  struct foxglove_string name, bool value);
#endif
 ...
 * - `name` must be a valid `foxglove_string`. This value is copied by this
 * function.
 * - `value` must be a valid `foxglove_string`. This value is copied by this
 * function.
 */
foxglove_error
foxglove_parameter_create_string(struct foxglove_parameter **param,
                                 struct foxglove_string name,
                                 struct foxglove_string value);
#endif
 ...
 * - `name` must be a valid `foxglove_string`. This value is copied by this
 * function.
 * - `value` must be a valid `foxglove_bytes`. This value is copied by this
 * function.
 */
foxglove_error
foxglove_parameter_create_byte_array(struct foxglove_parameter **param,
                                     struct foxglove_string name,
 ...
 * - `name` must be a valid `foxglove_string`. This value is copied by this
 * function.
 * - `values` must be a valid pointer to an array of float64 values of
 * `values_len` elements. This value is copied by this function.
 */
foxglove_error foxglove_parameter_create_float64_array(
    struct foxglove_parameter **param, struct foxglove_string name,
 ...
 */
foxglove_error foxglove_parameter_create_integer_array(
    struct foxglove_parameter **param, struct foxglove_string name,
    const int64_t *values, size_t values_len);
#endif
 ...
 *   `foxglove_parameter_value_dict_create`. This value is moved into this
 * function, and must not be accessed afterwards.
 */
foxglove_error
foxglove_parameter_create_dict(struct foxglove_parameter **param,
                               struct foxglove_string name,
                               struct foxglove_parameter_value_dict *dict);
#endif
 ...
 *
 * # Safety
 * - `param` must be a valid pointer to a value allocated by
 * `foxglove_parameter_create` or `foxglove_parameter_clone`.
 * - `size` must be a valid pointer.
 */
foxglove_error foxglove_parameter_get_byte_array_decoded_size(
    const struct foxglove_parameter *param, size_t *len);
#endif
 ...
 *
 * On success, updates `len` with the number of bytes written to the provided
 * buffer.
 *
 * # Safety
 * - `param` must be a valid pointer to a value allocated by
 ...
 */
foxglove_error
foxglove_parameter_decode_byte_array(const struct foxglove_parameter *param,
...
foxglove_parameter_clone(const struct foxglove_parameter *param);
#endif
 ...
 *
 * The value must be freed with `foxglove_parameter_value_free`, or by passing
 * it to a consuming function such as `foxglove_parameter_create`.
 */
struct foxglove_parameter_value *
foxglove_parameter_value_create_float64(double number);
#endif
 ...
 */
struct foxglove_parameter_value *
foxglove_parameter_value_create_integer(int64_t integer);
#endif
 ...
 */
struct foxglove_parameter_value *
foxglove_parameter_value_create_boolean(bool boolean);
#endif
 ...
 *
 * # Safety
 * - `string` must be a valid `foxglove_string`. This value is copied by this
 * function.
 */
foxglove_error
foxglove_parameter_value_create_string(struct foxglove_parameter_value **value,
                                       struct foxglove_string string);
#endif
 ...
 * # Safety
 * - `array` must be a valid pointer to a value allocated by
 *   `foxglove_parameter_value_array_create`. This value is moved into this
 * function, and must not be accessed afterwards.
 */
struct foxglove_parameter_value *foxglove_parameter_value_create_array(
    struct foxglove_parameter_value_array *array);
#endif
 ...
 * function, and must not be accessed afterwards.
 */
struct foxglove_parameter_value *foxglove_parameter_value_create_dict(
    struct foxglove_parameter_value_dict *dict);
#endif
 ...
 *
 * # Safety
 * - `value` must be a valid pointer to a value allocated by
 * `foxglove_parameter_value_create` or `foxglove_parameter_value_clone`.
 */
struct foxglove_parameter_value *
foxglove_parameter_value_clone(const struct foxglove_parameter_value *value);
#endif
 ...
 * # Safety
 * - `value` must be a valid pointer to a value allocated by
 * `foxglove_parameter_value_create_*`.
 */
 ...
 *
 * The parameter must be freed with `foxglove_parameter_value_array_free`, or by
 * passing it to a consuming function such as
 * `foxglove_parameter_value_create_array`.
 */
struct foxglove_parameter_value_array *
foxglove_parameter_value_array_create(size_t capacity);
#endif
 ...
 * - `value` must be a valid pointer to a value allocated by
 * `foxglove_parameter_value_create_*`. This value is moved into this function,
 * and must not be accessed afterwards.
 */
foxglove_error foxglove_parameter_value_array_push(
    struct foxglove_parameter_value_array *array,
 ...
 *
 * # Safety
 * - `array` is a valid pointer to a value allocated by
 * `foxglove_parameter_value_array_create`.
 */
void foxglove_parameter_value_array_free(
    struct foxglove_parameter_value_array *array);
#endif
 ...
 *
 * The parameter must be freed with `foxglove_parameter_value_dict_free`, or by
 * passing it to a consuming function such as
 * `foxglove_parameter_value_create_dict`.
 */
struct foxglove_parameter_value_dict *
foxglove_parameter_value_dict_create(size_t capacity);
#endif
 ...
 *
 * # Safety
 * - `key` must be a valid `foxglove_string`. This value is copied by this
 * function.
 * - `value` must be a valid pointer to a value allocated by
 * `foxglove_parameter_value_create_*`. This value is moved into this function,
 * and must not be accessed afterwards.
 */
foxglove_error
foxglove_parameter_value_dict_insert(struct foxglove_parameter_value_dict *dict,
                                     struct foxglove_string key,
                                     struct foxglove_parameter_value *value);
#endif
 ...
 *
 * # Safety
 * - `dict` is a valid pointer to a value allocated by
 * `foxglove_parameter_value_dict_create`.
 */
void foxglove_parameter_value_dict_free(
    struct foxglove_parameter_value_dict *dict);
#endif

#if !defined(__wasm__)
/**
 * Completes a `getParameters` request by sending parameter values to the
 * client.
 *
 * # Safety
 * - `responder` must be a pointer to a `foxglove_get_parameters_responder`
 * obtained via a `get` callback. This value is moved into this function, and
 * must not be accessed afterwards.
 * - `params` must be a valid pointer to a value allocated by
 * `foxglove_parameter_array_create`. This value is moved into this function,
 * and must not be accessed afterwards. A NULL value is treated as an empty
 * array.
 */
void foxglove_get_parameters_responder_respond(
    struct foxglove_get_parameters_responder *responder,
    struct foxglove_parameter_array *params);
#endif
 ...
 *
 * Reserved for unrecoverable internal errors; sends a generic error status to
 * the requesting client. In all other cases, complete the request with
 * `foxglove_get_parameters_responder_respond` (passing an empty array if no
 ...
 * must not be accessed afterwards.
 */
void foxglove_get_parameters_responder_drop(
    struct foxglove_get_parameters_responder *responder);
#endif

#if !defined(__wasm__)
/**
 * Completes a `setParameters` request with the values that were actually
 * applied.
 *
 * Echoes those values back to the requesting client when the request carried a
 * `request_id`; otherwise does nothing on the wire. Does not notify other
 * parameter subscribers; the caller is responsible for broadcasting applied
 * updates to subscribers on each sink (for example, via
 * `foxglove_server_publish_parameter_values` and
 * `foxglove_gateway_publish_parameter_values`).
 *
 * # Safety
 * - `responder` must be a pointer to a `foxglove_set_parameters_responder`
 * obtained via a `set` callback. This value is moved into this function, and
 * must not be accessed afterwards.
 * - `params` must be a valid pointer to a value allocated by
 * `foxglove_parameter_array_create`. This value is moved into this function,
 * and must not be accessed afterwards. A NULL value is treated as an empty
 * array.
 */
void foxglove_set_parameters_responder_respond(
    struct foxglove_set_parameters_responder *responder,
    struct foxglove_parameter_array *params);
#endif
 ...
 * Reserved for unrecoverable internal errors; sends a generic error status to
 * the requesting client. In all other cases, complete the request with
 * `foxglove_set_parameters_responder_respond` (passing an empty array if the
 ...
 * must not be accessed afterwards.
 */
void foxglove_set_parameters_responder_drop(
    struct foxglove_set_parameters_responder *responder);
#endif
 ...
 *
 * `port` may be 0, in which case an available port will be automatically
 * selected.
 *
 * Returns 0 on success, or returns a FoxgloveError code on error.
 *
 ...
 * - If `supported_encodings` is supplied in options, all `supported_encodings`
 * must contain valid UTF8, and `supported_encodings` must have length equal to
 * `supported_encodings_count`.
 * - If `server_info` is supplied in options, all `server_info` must contain
 * valid UTF8, and `server_info` must have length equal to `server_info_count`.
 */
 ...
 */
foxglove_error
foxglove_server_broadcast_time(const struct foxglove_websocket_server *server,
 ...
 *
 * # Safety
 * - `playback_state` must be a valid pointer to a playback state that lives for
 * the duration of the call.
 */
foxglove_error foxglove_server_broadcast_playback_state(
    const struct foxglove_websocket_server *server,
    const struct foxglove_playback_state *playback_state);
#endif

#if !defined(__wasm__)
/**
 * Sets a new session ID and notifies all clients, causing them to reset their
 * state.
 *
 ...
 */
foxglove_error
foxglove_server_clear_session(const struct foxglove_websocket_server *server,
                              const struct foxglove_string *session_id);
#endif
 ...
 *
 * This function will fail if the server was not configured with the `services`
 * capability, if a service with the same name is already registered, or if the
 * service has no request encoding and the server has no supported encodings.
 *
 * # Safety
 * - `server` must be a valid pointer to a server started with
 * `foxglove_server_start`.
 * - `service` must be a valid pointer to a service allocated by
 * `foxglove_service_create`. This value is moved into this function, and must
 * not be accessed afterwards.
 */
foxglove_error
foxglove_server_add_service(const struct foxglove_websocket_server *server,
                            struct foxglove_service *service);
#endif
 ...
 */
foxglove_error
foxglove_server_remove_service(const struct foxglove_websocket_server *server,
                               struct foxglove_string service_name);
#endif
 ...
 */
size_t
foxglove_server_get_client_count(struct foxglove_websocket_server *server);
#endif
 ...
 *
 * # Safety
 * - `params` must be a valid parameter to a value allocated by
 * `foxglove_parameter_array_create`. This value is moved into this function,
 * and must not be accessed afterwards.
 */
foxglove_error foxglove_server_publish_parameter_values(
    struct foxglove_websocket_server *server,
    struct foxglove_parameter_array *params);
#endif
 ...
 */
foxglove_error foxglove_server_publish_connection_graph(
    struct foxglove_websocket_server *server,
 ...
 *
 * The caller may optionally provide a message ID, which can be used in a
 * subsequent call to `foxglove_server_remove_status`.
 *
 * # Safety
 * - `message` must be a valid UTF-8 string, which must remain valid for the
 * duration of this call.
 ...
 */
foxglove_error
foxglove_server_publish_status(struct foxglove_websocket_server *server,
 ...
 */
foxglove_error
foxglove_server_remove_status(struct foxglove_websocket_server *server,
                              const struct foxglove_string *ids,
                              size_t ids_count);
#endif
 ...
 *
 * The service must be registered with a WebSocket server using
 * `foxglove_server_add_service`, or freed with `foxglove_service_free`.
 *
 * The callback is invoked from the client's main poll loop and must not block.
 * If blocking or long-running behavior is required, the implementation should
 * return immediately and handle the request asynchronously.
 *
 * The `request` structure provided to the callback is only valid for the
 * duration of the callback. If the implementation wishes to retain its data for
 * a longer lifetime, it must copy data out of it.
 *
 * The `responder` provided to the callback represents an unfulfilled response.
 * The implementation must eventually call either `foxglove_service_respond_ok`
 * or `foxglove_service_respond_error`, exactly once, in order to complete the
 * request. It is safe to invoke these completion functions synchronously from
 ...
 * - `callback` must be a valid pointer to a service callback function, which
 * must remain valid until the service is either unregistered or freed.
 */
foxglove_error foxglove_service_create(
    struct foxglove_service **service, struct foxglove_string name,
    const struct foxglove_service_schema *schema, const void *context,
 ...
 * # Safety
 * - `service` must be a valid pointer to a service allocated by
 * `foxglove_service_create`. The service MUST NOT have been previously
 * registered with a WebSocket server.
 */
 ...
 *
 * # Safety
 * - `responder` must be a pointer to a `foxglove_service_responder` obtained
 * via the `foxglove_service.handler` callback.
 * - `encoding` must be a valid UTF-8 string. This value is copied by this
 * function.
 */
foxglove_error foxglove_service_set_response_encoding(
    struct foxglove_service_responder *responder,
    struct foxglove_string encoding);
#endif
 ...
 * # Safety
 * - `responder` must be a pointer to a `foxglove_service_responder` obtained
 * via the `foxglove_service.handler` callback. This value is moved into this
 ...
 * function.
 */
void foxglove_service_respond_error(
    struct foxglove_service_responder *responder,
    struct foxglove_string message);
#endif
 ...
 *
 * On success, writes a non-null handle to `out_publisher`. On failure, returns
 * an error code and `out_publisher` is left untouched.
 *
 * The returned handle must be freed by calling either
 * [`foxglove_system_info_publisher_stop`] (to abort the background task) or
 * [`foxglove_system_info_publisher_detach`] (to leave the background task
 * running).
 *
 * # Safety
 * - `options` must be a valid pointer to a
 * [`FoxgloveSystemInfoPublisherOptions`] struct.
 * - `out_publisher` must be a valid, writable pointer to a `*mut
 * FoxgloveSystemInfoPublisher`.
 * - See the safety notes on [`FoxgloveSystemInfoPublisherOptions`].
 */
foxglove_error foxglove_system_info_publisher_start(
    const struct foxglove_system_info_publisher_options *options,
 ...
 *
 * This aborts the background task. After calling this function, the handle is
 * invalid and must not be used again. Passing a null pointer is a no-op.
 *
 * # Safety
 * - `publisher`, when non-null, must be a handle returned by
 *   [`foxglove_system_info_publisher_start`] that has not already been passed
 * to this function.
 */
foxglove_error foxglove_system_info_publisher_stop(
    struct foxglove_system_info_publisher *publisher);
#endif
 ...
 *
 * The background task continues to run until the process exits. After calling
 * this function, the handle is invalid and must not be used again. Passing a
 * null pointer is a no-op.
 *
 * # Safety
 * - `publisher`, when non-null, must be a handle returned by
 *   [`foxglove_system_info_publisher_start`] that has not already been passed
 * to either this function or [`foxglove_system_info_publisher_stop`].
 */
foxglove_error foxglove_system_info_publisher_detach(
    struct foxglove_system_info_publisher *publisher);
#endif
