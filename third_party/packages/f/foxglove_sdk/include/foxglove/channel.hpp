...
/// @brief A description of a channel. This will be constructed by the SDK and
/// passed to an implementation of a `SinkChannelFilterFn`.
...
public:
  ...
  explicit ChannelDescriptor(
      const foxglove_channel_descriptor *channel_descriptor);
  /// @endcond
...
/// Callables using the previous `bool(ChannelDescriptor&&)` signature are also
/// accepted but deprecated.
///
/// @return false if the channel should not be logged to the given sink. By
/// default, all channels are logged to a sink.
...
public:
  ...
  /// @deprecated Use a filter function taking `const ChannelDescriptor&`
  /// instead of `ChannelDescriptor&&`.
  template <typename F,
            typename = std::enable_if_t<
  ...
  [[deprecated("Use a filter function taking const ChannelDescriptor& instead "
               "of ChannelDescriptor&&")]]
  // NOLINTNEXTLINE(google-explicit-constructor,hicpp-explicit-conversions)
...
public:
  ...
  ///
  /// @param topic The topic name. You should choose a unique topic name per
  /// channel for compatibility with the Foxglove app.
  ...
  /// @param context The context which associates logs to a sink. If omitted,
  /// the default context is used.
  /// @param metadata Key/value metadata for the channel.
  static FoxgloveResult<RawChannel>
  create(const std::string_view &topic,
         const std::string_view &message_encoding,
  ...
  ///
  /// Zero-length messages are supported: pass `data_len == 0` with either a
  /// null `data` pointer or any non-null pointer. The contents of `data` are
  /// not read when `data_len == 0`.
  ///
  /// @param data The message data. May be null when `data_len == 0`.
  /// @param data_len The length of the message data, in bytes.
  /// @param log_time The timestamp of the message, as nanoseconds since epoch.
  /// If omitted, the current time is used.
  /// @param sink_id The sink ID associated with the message. Can be used to
  /// target logging messages to a specific client or mcap file. If omitted, the
  /// message is logged to all sinks. Note that providing a sink_id is not yet
  /// part of the public API. To partition logs among specific sinks, set up
  /// different `Context`s.
  FoxgloveError log(const std::byte *data, size_t data_len,
                    std::optional<uint64_t> log_time = std::nullopt,
                    std::optional<uint64_t> sink_id = std::nullopt) noexcept;

  /// @brief Close the channel.
  ///
  /// You can use this to explicitly unadvertise the channel to sinks that
  /// subscribe to channels dynamically, such as the WebSocketServer.
  ///
  /// Attempts to log on a closed channel will elicit a throttled warning
  /// message.
  void close() noexcept;
  ...
  ///
  /// @return The topic of the channel. The value is valid only for the lifetime
  /// of the channel.
  [[nodiscard]] std::string_view topic() const noexcept;

  /// @brief Get the message encoding of the channel.
  ///
  /// @return The message encoding of the channel. The value is valid only for
  ...
  // NOLINTNEXTLINE(readability-identifier-naming)
  [[deprecated("Use hasSinks() instead")]] [[nodiscard]] bool
  has_sinks() const noexcept {
    return hasSinks();
  }

  /// @brief Get the schema of the channel.
  ///
  /// @return The schema of the channel. The value is valid only for the
