...
private:
  ...
  void writeTo(foxglove_service_schema *c,
...
struct ServiceRequest {
  ...
  [[nodiscard]] std::string_view payloadStr() const noexcept {
    return {reinterpret_cast<const char *>(this->payload.data()),
            this->payload.size()};
  }
...
///
/// This callback is invoked from the client's main poll loop and must not
/// block. If blocking or long-running behavior is required, the implementation
/// should return immediately and handle the request asynchronously.
///
...
using ServiceHandler = std::function<void(const ServiceRequest &request,
...
public:
  ...
  static FoxgloveResult<Service>
  create(std::string_view name, ServiceSchema &schema, ServiceHandler &handler);
...
private:
  ...
  /// @brief Constructor from a raw pointer.
  explicit Service(foxglove_service *ptr) noexcept : impl_(ptr) {}
