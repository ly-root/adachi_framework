...
/// @brief A publisher that periodically logs process and system statistics on a
/// channel.
///
/// The publisher creates a channel on the configured Context (defaulting to
/// `/sysinfo`) and spawns a background task that logs a `SystemInfo` message at
/// the configured interval with stats on CPU and memory usage for the process
/// and the system.
///
...
///
/// - `process_memory` (number): Resident memory used by the SDK process, in
/// bytes.
/// - `process_virtual_memory` (number): Virtual memory used by the SDK process,
/// in bytes.
/// - `process_cpu_percent` (number): CPU usage for the SDK process, as a
/// percent of total
///   system CPU capacity (0.0 to 100.0).
/// - `process_cpu_cores` (number): CPU usage for the SDK process, expressed in
///   core-equivalents (0.0 to `num_cpus`). 1.0 means a single logical CPU is
///   fully utilized.
/// - `total_cpu_percent` (number): Total CPU usage across all logical CPUs on
/// the system,
///   as a percent (0.0 to 100.0).
/// - `total_cpu_cores` (number): Total CPU usage across the system, expressed
/// in
///   core-equivalents (0.0 to `num_cpus`). 1.0 means one logical CPU's worth of
///   work is being done.
...
///
/// The publisher runs until it is explicitly stopped via `stop()`. Destroying
/// this object does **not** stop the publisher: the background task continues
/// running until the process exits, matching the Rust and Python SDK behavior
/// of detaching the underlying task when the handle is dropped. Call `stop()`
/// explicitly to abort the background task before the process exits.
///
...
public:
  ...
  static FoxgloveResult<SystemInfoPublisher>
  create(SystemInfoOptions &&options = {});
  ...
  /// publisher to stop when this object goes out of scope, call stop()
  /// explicitly.
...
private:
  ...
  std::unique_ptr<foxglove_system_info_publisher,
                  foxglove_error (*)(foxglove_system_info_publisher *)>
      impl_;
};
