...
/// @brief An optional type hint for a `Parameter`, used to disambiguate values
/// whose intended type cannot be inferred from the wire representation alone.
///
/// A parameter's type is typically derived directly from its value: integers,
/// booleans, strings, dicts, and homogeneous arrays of these are unambiguous on
/// the wire. This enum only enumerates the cases that need an explicit hint:
///
/// - `ByteArray`: a byte array is transmitted as a base64-encoded string, so
/// without a type hint
///   it would be indistinguishable from an ordinary string.
/// - `Float64`: a whole-valued float (e.g. `1.0`) may be indistinguishable from
/// an integer on
///   the wire; the hint preserves the intended floating-point type.
/// - `Float64Array`: same rationale as `Float64`, for arrays.
///
/// Parameters of other types (integer, bool, string, dict, arrays of these)
/// leave `type()` as `ParameterType::None`.
enum class ParameterType : uint8_t {
  /// The parameter value can be inferred from the inner parameter value tag.
  /// Use this for
  /// parameters whose type is unambiguous on the wire (integer, bool, string,
  /// dict, or
  /// homogeneous arrays of these).
  None,
  /// A byte array, transmitted on the wire as a base64-encoded string. The type
  /// hint
  /// distinguishes it from an ordinary string value.
  ByteArray,
  /// A floating-point value that can be represented as a `float64`. Used to
  /// preserve the
  /// floating-point type for whole-valued numbers that would otherwise
...
public:
  ...
  using Value =
      std::variant<double, bool, int64_t, std::string_view, Array, Dict>;
...
template <>
[[nodiscard]] inline ParameterValueView
ParameterValueView::get<ParameterValueView>() const {
  return *this;
}
...
public:
  ...
  /// @brief Creates a deep clone of this parameter value.
  [[nodiscard]] ParameterValue clone() const { return this->view().clone(); }
...
public:
  ...
  template <typename T> [[nodiscard]] bool isArray() const noexcept {
    ...
    try {
      ...
      return std::all_of(
          arr.begin(), arr.end(),
          [](const ParameterValueView &elem) noexcept { return elem.is<T>(); });
    } catch (...) {
      return false;
    }
  }
  ...
  template <typename T> [[nodiscard]] bool isDict() const noexcept {
  ...
  template <typename T> [[nodiscard]] std::vector<T> getArray() const {
    auto value = this->value();
    if (!value) {
      throw std::bad_optional_access();
    }
  ...
  template <typename T> [[nodiscard]] std::map<std::string, T> getDict() const {
    auto value = this->value();
    if (!value) {
      throw std::bad_optional_access();
    }
...
private:
  ...
  /// @brief Constructor from raw pointer.
  explicit ParameterView(const foxglove_parameter *ptr) : impl_(ptr) {}
};

/// @brief Checks whether the paramter value is an array of generic elements.
template <>
[[nodiscard]] inline bool
ParameterView::isArray<ParameterValueView>() const noexcept {
  auto value = this->value();
...
template <>
[[nodiscard]] inline bool
ParameterView::isDict<ParameterValueView>() const noexcept {
  auto value = this->value();
...
/// @tparam std::vector<std::byte> Checks whether the parameter value is a byte
/// array.

/// @fn ParameterView::is<std::vector<double>>()
/// @tparam std::vector<double> Checks whether the parameter value is a vector
/// of floating point values.

/// @fn ParameterView::is<std::vector<int64_t>>()
/// @tparam std::vector<int64_t> Checks whether the parameter value is a vector
/// of integer values.
...
template <>
[[nodiscard]] inline bool
ParameterView::is<std::vector<std::byte>>() const noexcept {
  auto value = this->value();
  return value.has_value() && value->is<std::string_view>() &&
         this->type() == ParameterType::ByteArray;
}
...
template <>
[[nodiscard]] inline bool
ParameterView::is<std::vector<double>>() const noexcept {
...
template <>
[[nodiscard]] inline bool
ParameterView::is<std::vector<int64_t>>() const noexcept {
...
/// array.
template <>
[[nodiscard]] inline std::vector<ParameterValueView>
ParameterView::get() const {
...
/// dictionary.
template <>
[[nodiscard]] inline std::map<std::string, ParameterValueView>
ParameterView::get() const {
...
public:
  ...
  explicit Parameter(std::string_view name, const uint8_t *data,
                     size_t data_length);
  ...
  explicit Parameter(std::string_view name, const std::vector<std::byte> &bytes)
      : Parameter(name, reinterpret_cast<const uint8_t *>(bytes.data()),
                  bytes.size()) {}
  ...
  explicit Parameter(std::string_view name, ParameterType type,
  ...
  /// @brief Creates a deep clone of this parameter.
  [[nodiscard]] Parameter clone() const { return this->view().clone(); }
