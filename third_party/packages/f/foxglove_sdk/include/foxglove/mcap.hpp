...
struct CustomWriter {
  ...
  /// @param error Pointer to error code (set to an error number defined in
  /// errno.h if write fails)
...
struct Attachment {
  /// @brief Timestamp at which the attachment was recorded, in nanoseconds
  /// since epoch.
  uint64_t log_time = 0;
  /// @brief Timestamp at which the attachment was created, in nanoseconds since
  /// epoch. If not available, set to 0.
...
struct McapWriterOptions {
  ...
  /// @brief Whether to calculate and write a data section CRC into the DataEnd
  /// record.
  bool calculate_data_section_crc = true;
  /// @brief Whether to calculate and write a summary section CRC into the
  /// Footer record.
  ...
  /// @brief Compression level passed to the underlying compressor (zstd or
  /// lz4). A value of 0 instructs the compressor to use its default level.
...
public:
  ...
  ///
  /// @tparam Iterator An iterator type that dereferences to
  ...
  FoxgloveError writeMetadata(std::string_view name, Iterator begin,
                              Iterator end);

  /// @brief Write an attachment to the MCAP file.
  ///
  /// Attachments are arbitrary binary data that can be stored alongside
  /// messages. Common uses include storing configuration files, calibration
  /// data, or other reference material related to the recording.
  ///
  ...
  /// @brief Finishes the current chunk (if any) and flushes the underlying
  /// writer.
  ///
  /// Note that compression ratios tend to improve over the lifetime of a chunk,
  /// so flushing frequently with chunked output may reduce overall compression.
  ///
  /// @return FoxgloveError::Ok on success, or an error code on failure
  FoxgloveError flush();
...
private:
  ...
  std::unique_ptr<foxglove_mcap_writer,
                  foxglove_error (*)(foxglove_mcap_writer *)>
      impl_;
};

/// @copydoc McapWriter::writeMetadata
template <typename Iter>
FoxgloveError McapWriter::writeMetadata(std::string_view name, Iter begin,
                                        Iter end) {
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    // data and length for both are necessary because foxglove_string is a C
    // struct
  ...
  foxglove_error error = foxglove_mcap_write_metadata(
      impl_.get(), &c_name, c_metadata.data(), c_metadata.size());
...
///
/// @param position Pointer to the current write position, which must be kept up
/// to date by the
///   caller's write function.
/// @return A seek function suitable for @ref CustomWriter::seek.
/// @note This function is used to build a @ref CustomWriter for non-seekable
/// output destinations.
...
[[deprecated("Use noSeekFn() instead")]] inline SeekFunction
no_seek_fn(const uint64_t *position) {
