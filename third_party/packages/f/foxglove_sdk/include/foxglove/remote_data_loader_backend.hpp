...
///
/// Use @ref foxglove::remote_data_loader_backend::ChannelSet to declare
/// channels, then construct a
/// @ref foxglove::remote_data_loader_backend::StreamedSource with the resulting
/// topics and schemas. See @ref foxglove::remote_data_loader_backend::Manifest
/// for a full example.
///
/// @note This header requires [nlohmann/json](https://github.com/nlohmann/json)
/// and [tobiaslocker/base64](https://github.com/tobiaslocker/base64) to be
...
///
/// Schema data is stored as a base64-encoded string, matching the JSON wire
/// format.
struct Schema {
...
struct StreamedSource {
  ...
  ///
  /// You can provide a lower bound if this is not known exactly. This
  /// determines the start time of the seek bar in the Foxglove app.
...
inline std::string toJsonString(const Manifest &m) {
  ...
  auto source_to_json = [&](const StreamedSource &s) -> Json {
    ...
    Json j{
        {"url", s.url},          {"topics", topics},
        {"schemas", schemas},    {"startTime", s.start_time},
        {"endTime", s.end_time},
    };
    if (s.id.has_value()) {
      j["id"] = *s.id;
...
/// @brief A helper for building topic and schema metadata for a @ref
/// StreamedSource.
///
/// Handles schema extraction from Foxglove schema types, schema ID assignment,
/// and deduplication. If multiple channels share the same schema, only one
/// schema entry will be created.
///
/// @code{.cpp}
...
struct ChannelSet {
  ...
  template <typename T> void insert(const std::string &topic) {
  ...
  std::vector<Topic>
      topics; // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes)
  /// @brief The accumulated schemas (deduplicated).
  std::vector<Schema>
      schemas; // NOLINT(cppcoreguidelines-non-private-member-variables-in-classes)
...
private:
  ...
  static std::string encodeSchemaData(const foxglove::Schema &schema) {
    std::string_view sv(reinterpret_cast<const char *>(schema.data),
                        schema.data_len);
  ...
  uint16_t addSchema(const foxglove::Schema &schema) {
    ...
    if (next_schema_id_ == 0) {
      throw std::overflow_error(
          "ChannelSet: cannot add more than 65535 schemas");
    }
...
[[deprecated("Use toJsonString() instead")]] inline std::string
to_json_string(const Manifest &m) {
