...
enum class RemoteAccessConnectionStatus : uint8_t {
  ...
  ShuttingDown = 2,
  /// The gateway has been shut down. No further listener callbacks will be
  /// invoked.
  Shutdown = 3,
};
...
inline RemoteAccessGatewayCapabilities
operator|(RemoteAccessGatewayCapabilities a,
          RemoteAccessGatewayCapabilities b) {
...
struct RemoteAccessGatewayCallbacks {
  ...
  std::function<void(RemoteAccessConnectionStatus status)>
      onConnectionStatusChanged;

  /// @brief Callback invoked when a remote client subscribes to a channel.
  std::function<void(uint32_t client_id, const ChannelDescriptor &channel)>
      onSubscribe;

  /// @brief Callback invoked when a remote client unsubscribes from a channel
  /// or disconnects. Also invoked when a subscribed channel is removed from the
  /// Context.
  std::function<void(uint32_t client_id, const ChannelDescriptor &channel)>
      onUnsubscribe;
  ...
  /// @brief Callback invoked when a client subscribes to parameters for the
  /// first time.
  ///
  /// Requires RemoteAccessGatewayCapabilities::Parameters.
  ...
  /// @brief Callback invoked when the first client subscribes to connection
  /// graph updates.
  ///
  ...
  /// @brief Callback invoked when the last client unsubscribes from connection
  /// graph updates.
  ///
  /// Requires RemoteAccessGatewayCapabilities::ConnectionGraph.
  ///
...
enum class Reliability : uint8_t {
  ...
  Lossy = 0,
  /// Data is sent over the reliable control channel (ordered, guaranteed
  /// delivery).
  Reliable = 1,
};
...
struct RemoteAccessGatewayOptions {
  ...
  ///
  /// If empty, the token is read from the `FOXGLOVE_DEVICE_TOKEN` environment
  /// variable.
  std::string device_token;
  /// @brief Event callbacks.
  RemoteAccessGatewayCallbacks callbacks;
  /// @brief Advertised capabilities.
  RemoteAccessGatewayCapabilities capabilities =
      RemoteAccessGatewayCapabilities::None;
  ...
  ///
  /// If set, this callback is invoked for each channel to determine its
  /// quality-of-service profile. If not set, all channels use the default lossy
  /// profile.
  ...
  /// @brief (internal) Information about the gateway, which is shared with
  /// clients.
  ///
  ...
  ///
  /// Each participant gets an independent queue of this size. If a
  /// participant's queue fills up (because it is not reading fast enough), it
  /// will be disconnected and asked to reconnect.
  ///
  /// By default, each participant gets a queue of 1024 messages.
  std::optional<size_t> message_backlog_size = std::nullopt;
};

/// @brief A remote access gateway for live visualization and teleop in
/// Foxglove.
///
/// The gateway connects to the Foxglove platform and allows remote clients to
/// subscribe to channels and receive data.
///
/// @note RemoteAccessGateway is fully thread-safe, but
/// RemoteAccessGatewayCallbacks may be invoked concurrently from multiple
/// threads, so you will need to use synchronization in your callbacks.
class RemoteAccessGateway final {
public:
  /// @brief Create and start a gateway with the given options.
  static FoxgloveResult<RemoteAccessGateway>
  create(RemoteAccessGatewayOptions &&options);
  ...
  [[nodiscard]] FoxgloveError
  publishConnectionGraph(const ConnectionGraph &graph) const;
...
private:
  RemoteAccessGateway(foxglove_gateway *gateway,
  ...
  std::unique_ptr<foxglove_gateway, foxglove_error (*)(foxglove_gateway *)>
      impl_;
};
