...
/// A fixed-size memory arena that allocates aligned arrays of POD types on the
/// stack. The arena contains a single inline array and allocates from it. If
/// the arena runs out of space, it attempts to allocate the required space on
/// the heap. If this fails, it throws std::bad_alloc(). On wasm32 platforms
/// which do not support exceptions, It calls std::terminate(). The allocated
/// arrays are "freed" by dropping the arena, destructors are not run.
/// @cond foxglove_internal
class Arena {
public:
  static constexpr std::size_t kSize =
      static_cast<std::size_t>(8) * 1024; // 8 KB
  ...
  /// @return Pointer to the beginning of the allocated array of src.size() T
  /// elements, or null if elements is 0.
  /// @throws std::bad_alloc if the the fallback to heap allocation fails.
  /// On wasm32 platforms which do not support exceptions, calls
  /// std::terminate().
  template <
      typename T, typename S, typename Fn,
      typename = std::enable_if_t<
          std::is_pod_v<T> && std::is_invocable_v<Fn, T &, const S &, Arena &>>>
  ...
  /// Map a single source object of type S to a new object of type T allocated
  /// from the arena.
  ///
  ...
  /// On wasm32 platforms which do not support exceptions, calls
  /// std::terminate().
  template <typename T> T *alloc(size_t elements) {
    assert(elements > 0);
    ...
    void *aligned_ptr =
        std::align(alignment, bytes_needed, buffer_ptr, space_left);

    // Check if we have enough space
    if (aligned_ptr == nullptr) {
      // We don't use aligned_alloc because it fails on some platforms for
      // larger alignments
      ...
      aligned_ptr =
          std::align(alignment, bytes_needed, ptr, size_with_alignment);
      if (aligned_ptr == nullptr) {
#ifndef __wasm32__
        throw std::bad_alloc();
#else
  ...
  [[nodiscard]] size_t available() const { return kSize - offset_; }
