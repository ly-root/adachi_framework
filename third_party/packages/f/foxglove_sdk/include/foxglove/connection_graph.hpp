...
/// @brief A connection graph describing a topology of subscribers, publishers,
/// topics, and services.
///
/// Connection graph data can be published with
/// WebSocketServer::publishConnectionGraph or
/// RemoteAccessGateway::publishConnectionGraph, and visualized in the Foxglove
/// [Topic Graph panel].
///
/// [Topic Graph panel]:
...
public:
  ...
  FoxgloveError
  setAdvertisedService(std::string_view service,
                       const std::vector<std::string> &provider_ids);

private:
  std::unique_ptr<foxglove_connection_graph,
                  void (*)(foxglove_connection_graph *)>
      impl_;
};
