...
///
/// You can determine if the result is successful by checking `.has_value()`. If
/// the result is successful, error will be FoxgloveError::Ok and the expected
/// data can be unwrapped with
...
template <typename T> using FoxgloveResult = tl::expected<T, FoxgloveError>;
...
public:
  ...
  template <typename T> WarnStream &operator<<(const T &value) {
